#!/usr/bin/env python3
"""
SFC_SLP_Wind_TempF.py

Plot WRF sea level pressure (hPa), 10 m wind barbs (knots),
and 2 m temperature (°F) on a Cartopy map.
...
from wrf import ALL_TIMES, to_np  # ALL_TIMES kept for parity with v3 style
...
###############################################################################
# Surface SLP / Wind / TempF plotting for one (file, time_index) frame
###############################################################################
def process_frame(args):
    ...
    Physics / diagnostics:
        ...
        - 10 m winds, converted to knots and plotted as barbs.
        - 2 m temperature, smoothed and converted to °F.
        - Same contour levels, colormap, SLP ranges, etc. as original script.
    """
    ncfile_path, time_index, domain, path_figures = args

    # Open WRF file for this frame
    with Dataset(ncfile_path) as ncfile:
        ...
        # -------------------------------------------------------------------------
        # Physics: get variables (unchanged except timeidx made explicit)
        # -------------------------------------------------------------------------
        ...
        # Temp to °F (same formula as original: smooth in K, then convert)
        temp_f_np = (gaussian_filter(temp_np, sigma=1.0) - 273.15) * 9 / 5 + 32.0

        # Wind speed in knots (diagnostic, used in brightness masks)
        ...
        # -------------------------------------------------------------------------
        # Grid / projection: moving-nest safe
        # -------------------------------------------------------------------------
        ...
        # -------------------------------------------------------------------------
        # SLP smoothing (physics unchanged)
        # -------------------------------------------------------------------------
        ...
        # Map extent using extent_adjustment
        ax.set_extent(
            [
        ...
        # -------------------------------------------------------------------------
        # Temperature shading (°F) and SLP contours (physics unchanged)
        # -------------------------------------------------------------------------
        temp_levels = np.arange(-60, 121, 5)
        ...
        # SLP contour interval and range (unchanged)
        contour_interval = 2
        SLP_start = 870
        SLP_end = 1090
        ...
        temp_normalized = norm(temp_f_np)
        ...
        # Note: original script used T2 (in K) for this mask; we preserve that behavior
        outside_contour_mask = (
            (temp_np < temp_levels[0]) | (temp_np > temp_levels[-1]) | np.isnan(temp_np)
        )
        ...
        # Outside temp range -> black barbs
        ax.barbs(
        ...
        cbar2.set_label("Surface Temperature (°F)", fontsize=14)
        ...
        # -------------------------------------------------------------------------
        # Save figure with timestamped filename
        # -------------------------------------------------------------------------
        fname_time = valid_dt.strftime("%Y%m%d%H%M%S")
        file_out = f"wrf_{domain}_slp_wind_tempF_{fname_time}.png"
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 3:
        print(
            ...
            "For example:\n"
            "    SFC_SLP_Wind_TempF.py /home/WRF/test/em_real d01\n"
        )
        sys.exit(1)
    ...
    # Output directories
    # -------------------------------------------------------------------------
    path_figures = "wrf_SFC_TempF_figures"
    image_folder = os.path.join(path_figures, "Images")
    ...
    print("SFC plots (SLP/Wind/TempF) generation complete.")

    # -------------------------------------------------------------------------
    # GIF creation from sorted PNG files
    # -------------------------------------------------------------------------
