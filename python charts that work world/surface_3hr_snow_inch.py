#!/usr/bin/env python3
"""
Surface_3hrSnow_Inch_SLP_Isotherm.py

Plot WRF 3-hour accumulated snow (inches) using SNOWH,
...
def handle_domain_continuity_and_polar_mask(lats_np, lons_np, *fields):
    ...
    and optional masking for polar caps.

    Pass any number of fields; all are reordered/masked consistently.
    """
...
def process_frame(args):
    ...
    Physics:
        ...
        * Snow depth: wrf.getvar(..., "SNOWH", timeidx=time_index) (m) -> inches
        * 3-hour snow: current inches - inches from frame 2 steps back (as requested)
        * 850-hPa temp via wrf.vinterp with time-consistent field/pressure
    """
    (
        ncfile_path,
        time_index,
    ...
    with Dataset(ncfile_path) as ncfile:
        ...
        if prev2_ncfile_path is not None and prev2_time_index is not None:
            with Dataset(prev2_ncfile_path) as prev2_ncfile:
                ...
                prev2_total_snow_in = prev2_snowh_m * 39.3700787402
        ...
        # --- Numpy conversions + dateline/polar continuity ---
        slp_np = to_np(slp)
        three_hour_snow_np = to_np(three_hour_snow_in)
        temp_np = to_np(temp)
        ...
        cbar.set_label("3-hour Total Snow (in)", fontsize=14)
        ...
        file_out = f"wrf_{domain}_SLP_3hrSnow_Inch_{fname_time}.png"
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print(
            ...
            "For example:\n"
            "    Surface_3hrSnow_Inch_SLP_Isotherm_multicore_v3.py /home/WRF/test/em_real d01\n"
        )
        sys.exit(1)
    ...
    path_figures = "wrf_SFC_3hrSNOW_Inch_figures"
    image_folder = os.path.join(path_figures, "Images")
    ...
    # Discover all (file, time_index) pairs
    frames = discover_frames(ncfile_paths)
    if not frames:
        print("No timesteps found in provided WRF files.")
        sys.exit(0)

    # Sort frames by valid time so “idx-2” always means “two frames earlier in time”
    frames_with_time = []
    ...
    # Build args_list with “2 steps back” link (kept as requested)
    args_list = []
    ...
    print("Surface SLP and 3-hour snow (inch) plot generation complete.")
    ...
    gif_file_out = f"wrf_{domain}_3-hour_Total_Snow_Inch_SLP_Isotherm.gif"
