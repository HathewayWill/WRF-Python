#!/usr/bin/env python3
"""
WRF 700 hPa Wind, Heights and Isotachs (Multicore, Golden-style)

Usage
-----
python 700hPa_Wind_Height_Isotachs_multicorevgolden.py /path/to/wrf/run d01

Arguments
---------
path_wrf : directory that contains the wrfout_* files
domain   : WRF domain string (e.g. d01, d02)
"""
...
###############################################################################
# Isotach colormap configuration (same as 500 hPa golden)
###############################################################################
...
###############################################################################
# 700 hPa isotach and wind plotting for one (file, time_index) frame
###############################################################################
def process_frame(args):
    """
    Process a single (file, time_index) frame and save a PNG image.
    """
    ncfile_path, time_index, domain, path_figures = args

    # ------------------------------------------------------------------
    # Read data and compute diagnostics
    # ------------------------------------------------------------------
    with Dataset(ncfile_path) as nc:
        ...
        # Interpolate to 700 hPa
        u_700 = wrf.interplevel(u, p, 700)
        v_700 = wrf.interplevel(v, p, 700)
        z_700 = wrf.interplevel(z, p, 700)
        z_700 = gaussian_filter(to_np(z_700), sigma=2)

        # Convert to knots and compute wind speed
        u_700_knots = u_700 * 1.94384449
        v_700_knots = v_700 * 1.94384449
        wind_speed_knots = np.sqrt(u_700_knots**2 + v_700_knots**2)

        u_700_knots_np = to_np(u_700_knots)
        v_700_knots_np = to_np(v_700_knots)
        wind_speed_knots_np = to_np(wind_speed_knots)

        # Lat/lon grid at this time
        lats, lons = wrf.latlon_coords(u_700)

        # ------------------------------------------------------------------
        # Grid spacing, extents, cities, etc. (per-frame; safe for moving nests)
        # ------------------------------------------------------------------
        (
            lats_np,
            lons_np,
            ...
            avg_dy_km,
            extent_adjustment,
            _,  # label_adjustment (currently unused but kept for consistency)
        ...
        # Cartopy projection for this WRF domain
        cart_proj = wrf.get_cartopy(u_700)
        ...
        # Land and background features
        ax.add_feature(cfeature.LAND, facecolor=cfeature.COLORS["land"])
        for feat in features:
            add_feature(ax, *feat)

        # Cities and gridlines (canonical helpers)
        plot_cities(ax, lons_np, lats_np, avg_dx_km, avg_dy_km)
        add_latlon_gridlines(ax)

        # ------------------------------------------------------------------
        # 700 hPa geopotential height contours
        # ------------------------------------------------------------------
        z700_start = 200
        z700_end = 360
        contour_interval = 2
        height_levels = np.arange(z700_start, z700_end, contour_interval)
        ...
        # ------------------------------------------------------------------
        # 700 hPa wind speed isotachs (filled)
        # ------------------------------------------------------------------
        ...
        # ------------------------------------------------------------------
        # Wind barbs with brightness-aware coloring
        # ------------------------------------------------------------------
        ny, nx = u_700_knots_np.shape
        desired_barbs = 15
        ...
        ax.barbs(
            ...
            u_700_knots_np[::barb_density, ::barb_density][inside_contour_light_region],
            v_700_knots_np[::barb_density, ::barb_density][inside_contour_light_region],
            length=6,
        ...
        ax.barbs(
            ...
            u_700_knots_np[::barb_density, ::barb_density][inside_contour_dark_region],
            v_700_knots_np[::barb_density, ::barb_density][inside_contour_dark_region],
            length=6,
            sizes=dict(emptybarb=0.25, spacing=0.2, height=0.5),
            linewidth=0.8,
            color="lightgray",
        ...
        ax.barbs(
            ...
            u_700_knots_np[::barb_density, ::barb_density][outside_contour],
            v_700_knots_np[::barb_density, ::barb_density][outside_contour],
            length=6,
            sizes=dict(emptybarb=0.25, spacing=0.2, height=0.5),
            linewidth=0.8,
            color="black",
        ...
        file_out = f"wrf_{domain}_wind_700hPa_{ts}.png"
...
if __name__ == "__main__":
    ...
    path_figures = "wrf_700hPa_Isotachs_Wnd_Hgt"
    ...
    if not ncfile_paths:
        print(f"No wrfout files found under {path_wrf} for domain {domain}")
        sys.exit(1)

    # Build frame list (file, time_index)
    ...
    print("Wind barbs and isotachs at 700 hPa plot generation complete.")
    ...
    duration_ms = 800
    gif_file_out = f"wrf_{domain}_700hPa_WIND_Hgt_Isotachs.gif"
