#!/usr/bin/env python3
"""
Surface visibility (kilometers) from WRF output using AFWA-style vis_diagnostics logic.
...
def afwa_visibility_from_wrf(ncfile: Dataset, time_index: int):
    ...
    # Precipitable water (same diagnostic as original; no dust bins)
    try:
    ...
    # Dust extinction (assumed zero; AFWA uses 5-bin dust from WRF-Chem)
...
def create_gif(image_folder, animation_folder, domain):
    ...
    gif_file_out = f"wrf_{domain}_Surface_Visibility_km_AFWA.gif"
...
def process_frame(args):
    ...
    Steps:
        ...
        * Plot total visibility (km) as filled contours.
    ...
    with Dataset(ncfile_path) as ncfile:
        ...
        # Convert to km
        vis_total_km = vis_total_m / 1000.0
        ...
        lats_np, lons_np, vis_total_km, vis_alpha = (
        ...
        # -------------------------------------------------------------------------
        # Visibility ranges and colormap (physics/styling preserved)
        # -------------------------------------------------------------------------
        visibility_ranges_km = [
            0.00,
            0.50,
            0.75,
            1.25,
        ...
        visibility_norm = plt.matplotlib.colors.BoundaryNorm(
            visibility_ranges_km, visibility_map.N
        )
        ...
        cbar = plt.colorbar(
            ...
            pad=0.05,
            shrink=0.8,
            ticks=visibility_ranges_km,
        )
        cbar.set_label("Total Visibility (Kilometers)")
        cbar.ax.set_yticklabels([f"{lvl:.1f}" for lvl in visibility_ranges_km])
        ...
        file_out = f"wrf_{domain}_Surface_Visibility_km_AFWA_{fname_time}.png"
...
if __name__ == "__main__":
    ...
    path_figures = "wrf_Surface_Visibility_km_AFWA"
    ...
    print("AFWA Surface Visibility (km) plot generation complete.")
