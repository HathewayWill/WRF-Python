#!/usr/bin/env python3
"""
WRF 700 hPa RH, SLP, and 1000–500 hPa Thickness (Multicore, Golden-style)
"""
...
###############################################################################
# 700 hPa RH, SLP, and 1000–500 hPa Thickness plotting for one (file, time_index) frame
###############################################################################
def process_frame(args):
    ...
    with Dataset(ncfile_path) as nc:
        ...
        # Relative humidity and pressure/height fields
        rh = wrf.getvar(nc, "rh", timeidx=time_index)
        p = wrf.getvar(nc, "pressure", timeidx=time_index)
        geopotential_height = wrf.getvar(nc, "z", units="m", timeidx=time_index)

        # Interpolate RH to 700 hPa
        rh_700 = wrf.vinterp(
            nc,
            rh,
            "pressure",
        ...
        # Thickness: interpolate geopotential height to 500 and 1000 hPa
        new_levels = np.array([500, 1000])
        interpolated_geopotential_height = wrf.vinterp(
            nc,
            geopotential_height,
            "pressure",
            new_levels,
        ...
        pmin = interpolated_geopotential_height[0]  # 500 hPa
        pmax = interpolated_geopotential_height[1]  # 1000 hPa
        thickness = pmin - pmax

        # Lat/Lon for this frame (safe for moving nests)
        ...
        # Smooth SLP and geopotential height as in your original code
        geopotential_height_smooth = gaussian_filter(
            to_np(geopotential_height), sigma=2.0
        )
        SLP_smooth = gaussian_filter(to_np(SLP), sigma=5.0)
        ...
        # -------------------------------------------------------------------------
        if avg_dx_km >= 9 or avg_dy_km >= 9:
            contour_interval_slp = 4
        else:
            contour_interval_slp = 2

        SLP_start = 870
        SLP_end = 1090
        SLP_levels = np.arange(SLP_start, SLP_end, contour_interval_slp)
        ...
        rh_colors = ["#90EE90", "#00ff00", "#008000", "#006400"]

        rh_cmap = colors.ListedColormap(rh_colors)
        rh_norm = colors.BoundaryNorm(rh_levels, ncolors=rh_cmap.N)
        ...
        # -------------------------------------------------------------------------
        blue_contours = ax.contour(
            lons_np,
            lats_np,
            thickness,
            levels=np.arange(4800, 5400, 75),
            colors="blue",
            linewidths=1,
            linestyles="dashed",
            transform=crs.PlateCarree(),
        )
        ax.clabel(blue_contours, inline=True, fontsize=8)

        light_red_contours = ax.contour(
            lons_np,
            lats_np,
            thickness,
            levels=np.arange(5400, thickness.max(), 50),
            colors="coral",
            linewidths=1,
            linestyles="dashed",
            transform=crs.PlateCarree(),
        )
        ax.clabel(light_red_contours, inline=True, fontsize=8)

        red_contour = ax.contour(
            lons_np,
            lats_np,
            thickness,
            levels=[5400],
        ...
        ax.clabel(red_contour, inline=True, fontsize=11, fmt="%1.0f", colors="red")

        # -------------------------------------------------------------------------
        # High/low markers based on smoothed SLP
        # -------------------------------------------------------------------------
        smooth_slp = SLP_smooth
        ...
        # -------------------------------------------------------------------------
        # Save PNG with timestamp-based filename (golden for GIF sorting)
        # -------------------------------------------------------------------------
        image_folder = os.path.join(path_figures, "Images")
        os.makedirs(image_folder, exist_ok=True)

        ts = valid_dt.strftime("%Y%m%d%H%M%S")
        file_out = f"wrf_{domain}_SLP_rh_thickness_{ts}.png"
...
if __name__ == "__main__":
    ...
    path_figures = "wrf_700hPa_RH_SLP_Thickness"
    image_folder = os.path.join(path_figures, "Images")
    ...
    print("700 hPa Plot Generation Complete.")
    ...
    duration_ms = 800
    gif_file_out = f"wrf_{domain}_700hPa_RH_SLP_Thickness.gif"
