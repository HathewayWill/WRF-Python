...
Plot WRF 850-hPa wind barbs (knots), isotachs (knots),
...
This is recomputed per timestep, but uses the already-loaded `cities`
GeoDataFrame (loaded once per process).
...
###############################################################################
# 850 hPa isotach and wind plotting for one (file, time_index) frame
###############################################################################
def process_frame(args):
    ...
    Steps:
        ...
        * Plot:
            - 850 hPa heights (dm) as contours.
            - 850 hPa isotachs (knots) as filled contours.
            - 850 hPa wind barbs (knots), density adapted to grid size.
    ...
    with Dataset(ncfile_path) as ncfile:
        ...
        # -------------------------------------------------------------------------
        # Interpolate variables to 850 hPa and smooth height
        # -------------------------------------------------------------------------
        u_850 = wrf.vinterp(
            ncfile,
            u,
        ...
        # Smooth the 850-hPa geopotential height field (Gaussian filter)
        z_850 = gaussian_filter(to_np(z_850).squeeze(), sigma=2.0)
        ...
        u_850_knots_np = to_np(u_850_knots).squeeze()
        v_850_knots_np = to_np(v_850_knots).squeeze()

        wind_speed_knots = np.sqrt(u_850_knots_np**2 + v_850_knots_np**2)
        ...
        # -------------------------------------------------------------------------
        cart_proj = wrf.get_cartopy(u_850)  # WRF native projection for this domain/time
...
if __name__ == "__main__":
    ...
    # -------------------------------------------------------------------------
    path_figures = "wrf_850hPa_Isotachs_Wnd_Hgt"
    ...
    print("Wind barbs and isotachs at 850 hPa plot generation complete.")
    ...
    gif_file_out = f"wrf_{domain}_850hPa_WIND_Hgt_Isotachs.gif"
