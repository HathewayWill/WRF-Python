#!/usr/bin/env python3
"""
Surface_Wind_Chill_TempC.py

Plot WRF surface apparent / wind chill temperature (°C) with:
    * Sea level pressure (hPa) contours + H/L markers
    * Surface apparent / wind chill temperature (°C) filled contours
    * 0°C isotherm (solid blue line, using actual T2)
...
features = [
    ...
    # ("cultural", "10m", "none", "black", 0.60, "admin_2_counties", 2, 0.50),
...
###############################################################################
# Surface apparent / wind chill plotting for one (file, time_index) frame
###############################################################################
def process_frame(args):
    ...
    Steps:
        ...
        * Compute apparent / wind chill temperature field (°C).
        * Smooth SLP, find H/L centers and label.
        * Build map (features, cities, gridlines).
        * Plot:
            - SLP contours (hPa)
            - Apparent / wind chill (°C) as filled contours
            - 0°C isotherm (from actual T2)
    ...
    with Dataset(ncfile_path) as ncfile:
        ...
        # Total wind speed (knots)
        wind_speed_knots = np.sqrt(u10_knots**2 + v10_knots**2)

        # Convert wind speed to km/h for metric wind chill formula
        wind_speed_kmh = wind_speed_knots * 1.852  # 1 knot ≈ 1.852 km/h

        # Convert temperature to °C
        temp_c = temp - 273.15

        # -------------------------------------------------------------------------
        # Compute wind chill and combine with actual T where formula is invalid
        # (physics unchanged)
        # -------------------------------------------------------------------------
        # Metric wind chill index:
        #   WCI = 13.12 + 0.6215*T - 11.37*V**0.16 + 0.3965*T*V**0.16
        # where T (°C) ≤ 10 and V (km/h) ≥ 4.8
        wind_chill_c = (
            13.12
            + (0.6215 * temp_c)
        ...
        invalid = (temp_c > 10) | (wind_speed_kmh < 4.8)
        wind_chill_plot = np.where(invalid, temp_c, wind_chill_c)
        ...
        # -------------------------------------------------------------------------
        # Base map: land, features, cities, gridlines
        # -------------------------------------------------------------------------
        ...
        # Label SLP values below the symbols
        ax.text(
            min_lon,
        ...
        # -------------------------------------------------------------------------
        # Apparent / wind chill temperature color levels and colormap (°C)
        # -------------------------------------------------------------------------
        temp_levels = np.array(
            [
                -51,
                -48,
                -46,
        ...
        # 21 colors for 21 bins (-51..7); reuse first/last for under/over
        temp_map = plt.matplotlib.colors.ListedColormap(color_map_rgb)
        temp_map.set_under(color_map_rgb[0])  # < -51 °C
        temp_map.set_over(color_map_rgb[-1])  # > 7 °C

        temp_norm = plt.matplotlib.colors.BoundaryNorm(temp_levels, temp_map.N)

        # -------------------------------------------------------------------------
        # MSLP contour setup (unchanged physics)
        # -------------------------------------------------------------------------
        ...
        # -------------------------------------------------------------------------
        # Filled contours: apparent / wind chill temperature (°C)
        # -------------------------------------------------------------------------
        ...
        # -------------------------------------------------------------------------
        # 0°C isotherm from actual 2 m temperature
        # -------------------------------------------------------------------------
        temp_c_np = to_np(temp_c)
        ax.contour(
            lons_np,
            lats_np,
            temp_c_np,
        ...
        # -------------------------------------------------------------------------
        # Wind barbs: 10 m, knots, brightness-aware color
        # -------------------------------------------------------------------------
        ...
        # Barbs outside apparent temperature contour range (always black)
        ax.barbs(
        ...
        cbar2.set_label("Wind Chill Temperature (°C)", fontsize=14)
        cbar2.set_ticks(temp_levels)
        cbar2.ax.set_yticklabels([f"{int(t)}" for t in temp_levels])
        ...
        # -------------------------------------------------------------------------
        # Save PNG with valid_dt timestamp for GIF sorting
        # -------------------------------------------------------------------------
        fname_time = valid_dt.strftime("%Y%m%d%H%M%S")
        file_out = f"wrf_{domain}_SLP_WIND_WIND_CHILL_TEMPC_{fname_time}.png"
...
if __name__ == "__main__":
    ...
    # Output directories
    path_figures = "wrf_SFC_Wind_Chill_TempC_figures"
    ...
    print("SFC Wind Chill / Apparent Temperature plots complete.")

    # -------------------------------------------------------------------------
    # GIF generation from sorted PNGs
    # -------------------------------------------------------------------------
    ...
    gif_file_out = f"wrf_{domain}_SLP_WIND_WIND_CHILL_TEMPC.gif"
