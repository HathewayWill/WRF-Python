#!/usr/bin/env python3
"""
SFC total accumulated precipitation (mm) from WRF surface fields.

Computes:
    * Total precip from RAINC + RAINNC + RAINSH (mm)
    * Accumulated precip over the forecast period defined by:
        first wrfout_<domain>* file and last wrfout_<domain>* file.

Plots:
    * Total accumulated precipitation (mm) on a Cartopy map.
...
def process_frame(args):
    ...
    with Dataset(start_ncfile_path) as nc_start:
        ...
        start_total_rain = to_np(rainc_start + rainnc_start + rainsh_start)  # mm
    ...
    with Dataset(end_ncfile_path) as nc_end:
        ...
        end_total_rain = to_np(rainc_end + rainnc_end + rainsh_end)  # mm

        end_valid_dt = get_valid_time(nc_end, end_ncfile_path, end_time_index)

        # Accumulated rain over forecast period (physics identical)
        ...
        # Single, consistent time index (no ALL_TIMES)
        ...
        ###########################################################################
        # Contours and color map for total precip (mm)
        ###########################################################################
        # Smooth temp fields (kept even though not plotted directly)
        ...
        cbar = fig.colorbar(
            precip_contour, ax=ax, orientation="vertical", shrink=0.8, pad=0.05
        )
        cbar.set_label("Total Precipitation (mm)", fontsize=14)
        cbar.set_ticks(precip_levels)
        cbar.set_ticklabels([f"{level:.0f}" for level in precip_levels])
        ...
        # Titles (text preserved from original)
        plt.title(
...
if __name__ == "__main__":
    ...
    # Output directories
    path_figures = "wrf_SFC_Total_Precip_mm_figures"
