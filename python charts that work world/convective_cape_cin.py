...
Plot surface-based CAPE (J/kg) with CIN (J/kg) contours on a Cartopy map.
...
# Preserved from source script (non-v9 active layer) for reference:
# ("cultural", "10m", "none", "black", 0.60, "admin_2_counties", 2, 0.6),
...
###############################################################################
# CAPE/CIN plotting for one (file, time_index) frame
###############################################################################
def process_frame(args):
    """
    Process a single frame: one file and one time index.

    Physics preserved from original script:
        * cape_2d read via wrf.getvar.
        * CAPE = cape_2d[0, :]
        * CIN  = cape_2d[1, :]
        * CAPE masked between 0 and 500 J/kg.
        * Gaussian smoothing (sigma=1.0) for both CAPE and CIN.
        * CAPE levels 500–5500 every 500, custom colormap.
        * CIN dashed black contours, levels 0–950 every 50, negative labels.
    """
    ...
    with Dataset(ncfile_path) as ncfile:
        ...
        # -------------------------------------------------------------------------
        # Get CAPE/CIN field for this time index (physics invariant)
        # -------------------------------------------------------------------------
        cape_2d = wrf.getvar(ncfile, "cape_2d", timeidx=time_index)

        # Use the full field (with coords) for lat/lon and projection
        lats, lons = wrf.latlon_coords(cape_2d)
        cart_proj = wrf.get_cartopy(cape_2d)

        # Slice to CAPE and CIN exactly like the original physics
        cape = cape_2d[0, :]
        cin = cape_2d[1, :]

        # Mask and smooth CAPE/CIN exactly as before
        cape = np.ma.masked_where((cape > 0) & (cape < 500), cape)
        cape = gaussian_filter(cape, sigma=1.0)
        cin = gaussian_filter(cin, sigma=1.0)

        # -------------------------------------------------------------------------
        # Grid spacing and extent (moving-nest safe)
        # -------------------------------------------------------------------------
        (
            lats_np,
            lons_np,
        ...
        # Map extent with padding based on resolution
        ax.set_extent(
            [
        ...
        # -------------------------------------------------------------------------
        # CAPE filled contours (physics: levels and colormap preserved)
        # -------------------------------------------------------------------------
        colors_custom = [
            (0.93, 0.93, 0.93),  # light grey (0-500)
            (0.0, 1.0, 0.4),  # neon green (1000-1500)
            (1.0, 1.0, 0.0),  # yellow (1500-2000)
            (1.0, 0.6, 0.0),  # yellow orange (2000-3500)
            (1.0, 0.4, 0.0),  # orange (3500-4000)
            (1.0, 0.0, 0.0),  # red (4500-5000)
            (1.0, 0.0, 1.0),  # pink (>5500)
        ]

        cape_levels = np.arange(500, 6000, 500)

        cmap_custom = mcolors.LinearSegmentedColormap.from_list(
            "custom_cape_cmap", colors_custom, N=len(cape_levels)
        )

        cape_contourf = ax.contourf(
            lons_np,
            lats_np,
            to_np(cape_plot),
            levels=cape_levels,
        ...
        # -------------------------------------------------------------------------
        # CIN dashed contours (physics: levels, style, labels preserved)
        # -------------------------------------------------------------------------
        cin_levels = np.arange(0, 1000, 50, dtype=float)
        ...
        cbar = plt.colorbar(cape_contourf, ax=ax, orientation="vertical", pad=0.05)
        cbar.ax.set_ylabel("CAPE (J/kg)", rotation=270, labelpad=15, fontsize=14)
        ...
        file_out = f"wrf_{domain}_CAPE_CIN_{fname_time}.png"
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 3:
        print(
            ...
            "For example:\n"
            "    sfc_CAPE_CIN.py /home/WRF/test/em_real d01\n"
        )
        sys.exit(1)
    ...
    print("CAPE and CIN plot generation complete.")
    ...
    gif_file_out = f"wrf_{domain}_CAPE_CIN.gif"
