#!/usr/bin/env python3
"""
High_Cloud_Cover_meters.py

Plot WRF high cloud fraction (%) based on height above ground (meters),
using wrf.cloudfrac thresholds:
...
def process_frame(args):
    ...
    Steps:
        * Read WRF variables (height_agl, rh) at this time.
        * Compute wrf.cloudfrac with thresholds in meters.
        * Extract high cloud fraction (%).
        * Build map (features, cities, gridlines).
        * Plot high cloud fraction as filled contours.
    ...
    with Dataset(ncfile_path) as ncfile:
        ...
        # -------------------------------------------------------------------------
        # Physics / diagnostics block (unchanged in spirit)
        # -------------------------------------------------------------------------
        ...
        (
            ...
            avg_dy_km,
            extent_adjustment,
            label_adjustment,  # kept for possible label offsets, currently unused
        ...
        # Get the cartopy mapping object from WRF field
        ...
        # -------------------------------------------------------------------------
        # High cloud fraction filled contours
        # -------------------------------------------------------------------------
        clevels = np.arange(5, 100, 5)  # 5% to 95% in 5% increments
        ...
        plt.title(
            ...
            f"High Cloud Levels (above 5400 m)",
            loc="left",
            fontsize=13,
        )
        plt.title(
...
if __name__ == "__main__":
    ...
    # -------------------------------------------------------------------------
    path_figures = "wrf_Cloud_Cover_High_Meters"
    ...
    gif_file_out = f"wrf_{domain}_High_Cloud_Cover_Meters.gif"
