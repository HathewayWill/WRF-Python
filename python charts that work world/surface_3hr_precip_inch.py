#!/usr/bin/env python3
"""
SFC_3hrPrecip_SLP_multicore_v3.py

Plot WRF 3-hour accumulated precipitation (inches) and
mean sea level pressure (hPa) on a Cartopy map.
...
###############################################################################
# 3-hour precipitation + SLP plotting for one (file, time_index) frame
###############################################################################
def process_frame(args):
    ...
    Steps:
        * Read WRF variables (SLP, RAINC, RAINNC, RAINSH, T2, temp).
        * Compute 3-hour accumulated precip (current minus t-3h cumulative).
        * Compute 850-hPa temperature using wrf.vinterp (unchanged physics).
        * Smooth SLP and temperature fields.
        * Build map (features, cities, gridlines).
        * Plot:
            - SLP (hPa) as contours.
            - 3-hour precip (inches) as filled contours.
    ...
    with Dataset(ncfile_path) as ncfile:
        ...
        # Valid time from WRF metadata / filename
        ...
        # Cumulative precip (mm) → inches (0.0393701)
        total_rain = (rainc + rainnc + rainsh) * 0.0393701

        # 3-hour accumulated precip: current cumulative minus cumulative 3 frames back
        if prev3_ncfile_path is not None and prev3_time_index is not None:
            with Dataset(prev3_ncfile_path) as prev_ncfile:
                prev_rainc = wrf.getvar(prev_ncfile, "RAINC", timeidx=prev3_time_index)
                prev_rainnc = wrf.getvar(
                    prev_ncfile, "RAINNC", timeidx=prev3_time_index
                )
                prev_rainsh = wrf.getvar(
                    prev_ncfile, "RAINSH", timeidx=prev3_time_index
                )
                prev_total_rain = (prev_rainc + prev_rainnc + prev_rainsh) * 0.0393701

            three_hour_rain = total_rain - prev_total_rain
        else:
        ...
        # -------------------------------------------------------------------------
        # Coordinates / grid spacing (per-frame, moving-nest safe)
        # -------------------------------------------------------------------------
        ...
        cbar.set_label("3-hour Total Precipitation (in)", fontsize=14)
        ...
        # -------------------------------------------------------------------------
        # Save PNG with valid-time stamp for GIF sorting
        # -------------------------------------------------------------------------
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 3:
        print(
            ...
            "For example:\n"
            "    SFC_3hrPrecip_SLP_multicore_v3.py /home/WRF/test/em_real d01\n"
        )
        sys.exit(1)
    ...
    # -------------------------------------------------------------------------
    path_figures = "wrf_SFC_3hrPrecip_Inch_figures"
    image_folder = os.path.join(path_figures, "Images")
    ...
    # Sort frames by valid time to define the global chronological sequence
    frames_with_time = []
    ...
    # -------------------------------------------------------------------------
    # Build 3-hour backward links (index-3) in the sorted global time sequence
    # -------------------------------------------------------------------------
    args_list = []
    for idx, (ncfile_path, time_index) in enumerate(sorted_frames):
        if idx >= 3:
            prev3_ncfile_path, prev3_time_index = sorted_frames[idx - 3]
        else:
            prev3_ncfile_path, prev3_time_index = (None, None)
    ...
    # Skip first two frames (no full 3-hour accumulation), as in original script
    if len(png_files_sorted) <= 2:
        print("Not enough frames for 3-hour accumulation GIF. Skipping GIF step.")
        sys.exit(0)
    ...
    images = [
        ...
        for filename in png_files_for_gif
    ]
    if not images:
