...
Computes (physics / diagnostics unchanged):
    * Total snow depth from SNOWH (m) converted to inches.
    * Accumulated snow over the forecast period defined by:
        first (wrfout file, time_index) frame and last (wrfout file, time_index) frame.

Plots:
    * Total accumulated snow (inches) on a Cartopy map.
"""
...
###############################################################################
# Total accumulated snow plotting for one (start_frame, end_frame) product
###############################################################################
def process_frame(args):
    ...
    Physics identical:
        snow = wrf.getvar(ncfile, "SNOWH")
        total_snow = snow * 39.3700787402 (m → inches)
    ...
    # -------------------------------------------------------------------------
    # Compute total snow at start and end frames (physics identical)
    # -------------------------------------------------------------------------
    with Dataset(start_ncfile_path) as nc_start:
        snow_start = wrf.getvar(nc_start, "SNOWH", timeidx=start_time_index)  # m
        start_total_snow = to_np(snow_start * 39.3700787402)  # inches
        start_valid_dt = get_valid_time(nc_start, start_ncfile_path, start_time_index)

    with Dataset(end_ncfile_path) as nc_end:
        snow_end = wrf.getvar(nc_end, "SNOWH", timeidx=end_time_index)  # m
        ...
        # ---------------------------------------------------------------------
        # IMPORTANT:
        #   Keep 'temp' as a WRF var (xarray-like) so get_cartopy/latlon_coords work.
        temp = wrf.getvar(nc_end, "T2", timeidx=end_time_index)

        # Single, consistent time index (no ALL_TIMES) to avoid shape mismatch
        ...
        # Lat/lon + grid spacing (recomputed per frame)
        ...
        ###########################################################################
        # Contours and color map for total snow (inches)
        ###########################################################################
        # Smooth temp fields (kept as in original, even if not contoured)
        ...
        # Map extent with padding (per-frame)
        ax.set_extent(
            [
        ...
        # Base land feature + additional features
        ...
        # 0 to 36 inch per 24 hour colorbar (unchanged)
        ...
        # Titles (kept in spirit of original)
        plt.title(
    ...
    print("SFC Total Snow Inch plots complete")
