#!/usr/bin/env python3
"""
SFC_1hrSnow_SLP_Inch.py

Plot WRF mean sea-level pressure (SLP, hPa) and 1-hour accumulated
snowfall (inches, from SNOWH) on a Cartopy map.
...
from wrf import ALL_TIMES, to_np  # ALL_TIMES imported for completeness
...
###############################################################################
# SLP + 1-hour snow plotting for one (file, time_index) frame
###############################################################################
def process_frame(args):
    ...
    Steps:
        ...
        * Compute 1-hour accumulation as difference of cumulative totals (inches).
        * Build map (features, cities, gridlines).
        * Plot:
            - Smoothed SLP contours (hPa) with H/L markers.
            - 1-hour accumulated snow (inches) as filled contours.
    ...
    with Dataset(ncfile_path) as ncfile:
        ...
        snow = wrf.getvar(ncfile, "SNOWH", timeidx=time_index)  # meters
        total_snow = snow * 39.3700787402  # meters → inches
        ...
        if prev_ncfile_path is not None and prev_time_index is not None:
            with Dataset(prev_ncfile_path) as prev_ncfile:
                prev_snow = wrf.getvar(prev_ncfile, "SNOWH", timeidx=prev_time_index)
                prev_total_snow = prev_snow * 39.3700787402  # meters → inches
        ...
        # Ensure snow field is numpy for continuity helper + plotting
        ...
        contour_interval = 2  # fixed, as in original
        SLP_start = 870
        SLP_end = 1090
        ...
        # -------------------------------------------------------------------------
        # 1-hour accumulated snow (inches) filled contours
        # -------------------------------------------------------------------------
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 3:
        print(
            ...
            "For example:\n"
            "    SFC_1hrSnow_SLP_Inch.py /home/WRF/test/em_real d01\n"
        )
        sys.exit(1)
    ...
    # -------------------------------------------------------------------------
    path_figures = "wrf_SFC_1hrSNOW_Inch_figures"
    image_folder = os.path.join(path_figures, "Images")
    ...
    print("SFC SLP + 1-hour snow (inches) plot generation complete.")
