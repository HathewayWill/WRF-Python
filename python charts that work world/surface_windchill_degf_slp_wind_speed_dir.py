#!/usr/bin/env python3
"""
WRF SURFACE WIND CHILL / APPARENT TEMPERATURE PLOTTER (°F) – v3

- Reads WRF output files (wrfout_*)
- Computes 2 m wind chill in °F using the NWS formula
- Where the wind chill formula is invalid (T > 50°F or V < 3 mph),
  uses the actual 2 m temperature instead of masking
- Plots:
    * Sea level pressure (hPa) contours + H/L markers
    * Surface apparent / wind chill temperature (°F) filled contours
    * 32°F isotherm (solid blue line, using actual T2)
    * 10 m wind barbs (knots) with color-dependent visibility
    * Cities from Natural Earth database
- Writes one PNG per (file, time_index) and then builds an animated GIF
"""
...
###############################################################################
# Surface wind chill / apparent temperature (°F) for one (file, time_index) frame
###############################################################################
def process_frame(args):
    ...
    Steps:
        * Read WRF variables (slp, U10, V10, T2) at this time.
        * Compute NWS wind chill in °F, merge with T where invalid.
        * Smooth SLP, find H/L centers.
        * Build map (features, cities, gridlines).
        * Plot:
            - SLP contours (hPa) + H/L markers.
            - Apparent / wind chill temperature (°F) filled contours.
            - 32°F isotherm from T2.
            - 10 m wind barbs (knots) with color-dependent visibility.
    ...
    with Dataset(ncfile_path) as ncfile:
        ...
        ###########################################################################
        slp = wrf.getvar(ncfile, "slp", timeidx=time_index)  # sea level pressure
        u10 = wrf.getvar(ncfile, "U10", timeidx=time_index)  # 10 m U wind (m/s)
        v10 = wrf.getvar(ncfile, "V10", timeidx=time_index)  # 10 m V wind (m/s)
        temp = wrf.getvar(ncfile, "T2", timeidx=time_index)  # 2 m temperature (K)
        ...
        # Wind speed in mph for NWS wind chill formula
        wind_speed_mph = wind_speed_knots * 1.15078  # 1 kt ≈ 1.15078 mph

        # Temperature in °F
        temp_f = (temp - 273.15) * 9.0 / 5.0 + 32.0

        ###########################################################################
        # NWS wind chill (°F) and apparent temperature field
        ###########################################################################
        # NWS wind chill index:
        #   WCI = 35.74 + 0.6215*T - 35.75*V**0.16 + 0.4275*T*V**0.16
        # where T (°F) ≤ 50 and V (mph) ≥ 3
        wind_chill_f = (
            35.74
            + 0.6215 * temp_f
        ...
        invalid = (temp_f > 40.0) | (wind_speed_mph < 3.0)

        # Physics intent: use actual T where invalid, wind chill elsewhere
        wind_chill_plot = np.where(invalid, temp_f, wind_chill_f)

        wind_chill_plot_np = to_np(wind_chill_plot)
        temp_f_np = to_np(temp_f)

        u10_knots_np = to_np(u10_knots)
        v10_knots_np = to_np(v10_knots)

        ###########################################################################
        # Coordinates, projection, and grid spacing (moving-nest safe)
        ###########################################################################
        ...
        ###########################################################################
        # Sea level pressure (smoothed) and H/L markers
        ###########################################################################
        ...
        ###########################################################################
        # Apparent / wind chill temperature levels and colormap (°F)
        ###########################################################################
        # 5°F bin edges from -60 to 40 → 21 boundaries ⇒ 20 bins
        ...
        color_map_rgb = (
            np.array(
                [
                    ...
                    [206, 18, 86],  # -60 to -55
                    [231, 41, 138],  # -55 to -50
                    [223, 101, 176],  # -50 to -45
                    [255, 115, 223],  # -45 to -40
                    [255, 190, 232],  # -40 to -35
                    [255, 255, 255],  # -35 to -30
                    [218, 218, 235],  # -30 to -25
                    [188, 189, 220],  # -25 to -20
                    [158, 154, 200],  # -20 to -15
                    [117, 107, 177],  # -15 to -10
                    [84, 39, 143],  # -10 to -5
                    [13, 0, 125],  # -5 to 0
                    [13, 61, 156],  # 0 to 5
                    [0, 102, 194],  # 5 to 10
                    [41, 158, 255],  # 10 to 15
                    [74, 199, 255],  # 15 to 20
                    [115, 215, 255],  # 20 to 25
                    [173, 255, 255],  # 25 to 30
                    [48, 207, 194],  # 30 to 35
                    [0, 153, 150],  # 35 to 40
                ],
                np.float32,
        ...
        # 20 colors for the 20 bins (-60..40). First color is used as "under".
        temp_map = plt.matplotlib.colors.ListedColormap(color_map_rgb[1:])
        temp_map.set_under(color_map_rgb[0])  # < -60
        temp_map.set_over(color_map_rgb[-1])  # > 40 (same as 35–40)
        ...
        # MSLP contours
        ###########################################################################
        contour_interval = 2  # hPa
        mslp_start = 870
        mslp_end = 1090
        mslp_levels = np.arange(mslp_start, mslp_end, contour_interval)
        ...
        ax.clabel(mslp_contours, inline=1, fontsize=10, fmt="%1.0f")

        ###########################################################################
        # Filled contours: apparent / wind chill temperature (°F)
        ###########################################################################
        ...
        ###########################################################################
        # 32°F isotherm from actual 2 m temperature
        ###########################################################################
        temp_32deg_line = ax.contour(
            lons_np,
            lats_np,
            temp_f_np,
            levels=[32.0],
        ...
        plt.clabel(mslp_contours, inline=True, fontsize=12, fmt="%d", colors="k")

        ###########################################################################
        # Wind barbs (10 m, knots) with contrast-aware coloring
        ###########################################################################
        ...
        norm_for_barbs = plt.Normalize(vmin=temp_levels[0], vmax=temp_levels[-1])
        temp_normalized = norm_for_barbs(wind_chill_plot_np)

        brightness_map = np.interp(
            temp_normalized, np.linspace(0, 1, len(color_brightness)), color_brightness
        )
        ...
        outside_contour_mask = (
            (wind_chill_plot_np < temp_levels[0])
            | (wind_chill_plot_np > temp_levels[-1])
            | np.isnan(wind_chill_plot_np)
        )
        ...
        # Barbs over light regions (dark barbs)
        ax.barbs(
        ...
        # Barbs over dark regions (light barbs)
        ax.barbs(
            lons_ds[inside_dark],
            lats_ds[inside_dark],
            u_ds[inside_dark],
        ...
        # Barbs outside contour range (always black)
        ax.barbs(
        ...
        ###########################################################################
        cbar2 = fig.colorbar(
            temperature, ax=ax, orientation="vertical", shrink=0.8, pad=0.05
        )
        cbar2.set_label("Wind Chill Temperature (°F)", fontsize=14)
        tick_values = temp_levels
        cbar2.set_ticks(tick_values)
        cbar2.ax.set_yticklabels([f"{int(t)}" for t in tick_values])
        ...
        file_out = f"wrf_{domain}_slp_wind_wind_chill_tempF_{fname_time}.png"

        plt.savefig(
            os.path.join(path_figures, "Images", file_out), bbox_inches="tight", dpi=250
        )
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print(
            ...
            "Example: script_name.py /home/WRF/test/em_real d01\n"
        )
        sys.exit(1)
    ...
    # Output directories
    path_figures = "wrf_SFC_Wind_Chill_TempF_figures"
    ...
    # Find all WRF output files and discover frames
    ...
    ###########################################################################
    # Animated GIF creation from sorted PNG files
    ###########################################################################
    ...
    # Filenames contain YYYYMMDDHHMMSS → alphabetical sort is chronological
    ...
    gif_file_out = f"wrf_{domain}_SLP_WIND_WIND_CHILL_TEMPF.gif"
