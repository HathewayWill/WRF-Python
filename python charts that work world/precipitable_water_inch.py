...
Plot WRF column precipitable water (inches) on a Cartopy map.
...
It does NOT assume the domain is static:
    * For each timestep, lat/lon, grid spacing, and extent are recomputed
      from the WRF fields. This automatically works for both static nests
      and moving/vortex-following nests.
"""
...
###############################################################################
# Precipitable water plotting for one (file, time_index) frame
###############################################################################
def process_frame(args):
    ...
    Steps:
        * Read WRF precipitable water at this time.
        * Convert to inches.
        * Build map (features, cities, gridlines).
        * Plot precipitable water as filled contours.
    ...
    with Dataset(ncfile_path) as ncfile:
        ...
        # -------------------------------------------------------------------------
        # Physics: precipitable water (unchanged)
        # -------------------------------------------------------------------------
        # 1 kg/m^2 = 1 mm H2O
        precipitable_water = wrf.getvar(ncfile, "pw", timeidx=time_index)

        # Convert to inches (original factor)
        precipitable_water_in = precipitable_water * 0.0393701
        precipitable_water_in_np = to_np(precipitable_water_in)

        # -------------------------------------------------------------------------
        # Grid / geometry (moving-nest safe)
        # -------------------------------------------------------------------------
        lats, lons = wrf.latlon_coords(precipitable_water)
        (
            lats_np,
            lons_np,
            avg_dx_km,
        ...
        cart_proj = wrf.get_cartopy(precipitable_water)
        ...
        # -------------------------------------------------------------------------
        # Precipitable water contours (inches) — physics levels unchanged
        # -------------------------------------------------------------------------
        precip_levels = np.array([0.00, 0.25, 0.50, 1.00, 1.50, 2.00, 2.50, 3.00, 3.50])
        ...
        cbar.set_label("Precipitable Water (in)", fontsize=14)
        ...
        # Filename with timestamp for correct GIF sorting
        fname_time = valid_dt.strftime("%Y%m%d%H%M%S")
        file_out = f"wrf_{domain}_PW_in_{fname_time}.png"
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 3:
        print(
            ...
            "For example:\n"
            "    Precipitable_Water_Inch.py /home/WRF/test/em_real d01\n"
        )
        sys.exit(1)
    ...
    # -------------------------------------------------------------------------
    path_figures = "wrf_Precipitable_Water_Inch"
    image_folder = os.path.join(path_figures, "Images")
    ...
    print("Precipitable water (inches) plot generation complete.")
    ...
    gif_file_out = f"wrf_{domain}_Precipitable_Water.gif"
