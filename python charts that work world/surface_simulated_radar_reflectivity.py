#!/usr/bin/env python3
"""
SFC_Simulated_Composite_Reflectivity.py

Plot WRF simulated composite reflectivity (dBZ) on a Cartopy map.
...
from wrf import ALL_TIMES, to_np  # ALL_TIMES imported for consistency
...
###############################################################################
# Grid, spacing, and extent (per frame, moving-nest safe)
###############################################################################
"""
def compute_grid_and_spacing(lats, lons):
    "
...
###############################################################################
# SFC simulated composite reflectivity plotting for one frame
###############################################################################
def process_frame(args):
    ...
    Steps:
        * Read WRF mdbz at this time.
        * Build map (features, cities, gridlines).
        * Plot:
            - Simulated composite reflectivity (dBZ) as filled contours.
    ...
    with Dataset(ncfile_path) as ncfile:
        ...
        # -------------------------------------------------------------------------
        # Get reflectivity field (physics invariant)
        # -------------------------------------------------------------------------
        dbz_max = wrf.getvar(ncfile, "mdbz", timeidx=time_index)
        if dbz_max.ndim == 3:
            dbz_max = dbz_max[0, :, :]

        # -------------------------------------------------------------------------
        # Lat/lon and grid spacing per frame (moving-nest safe)
        # -------------------------------------------------------------------------
        lats, lons = wrf.latlon_coords(dbz_max)
        (
            lats_np,
            lons_np,
            avg_dx_km,
            avg_dy_km,
            extent_adjustment,
            label_adjustment,  # currently unused but returned for consistency
        ) = compute_grid_and_spacing(lats, lons)

        # -------------------------------------------------------------------------
        # Dateline continuity and polar masking (v9 canonical helper)
        # -------------------------------------------------------------------------
        (lats_np, lons_np, dbz_max_np) = handle_domain_continuity_and_polar_mask(
            lats_np,
            lons_np,
            to_np(dbz_max),
        )
        ...
        # Cartopy projection
        cart_proj = wrf.get_cartopy(dbz_max)

        # -------------------------------------------------------------------------
        # Reflectivity levels and color table (physics invariant)
        # -------------------------------------------------------------------------
        ...
        dbz_map = plt.matplotlib.colors.ListedColormap(dbz_rgb[:-1])
        dbz_map.set_over(dbz_rgb[-1])
        dbz_norm = plt.matplotlib.colors.BoundaryNorm(dbz_levels, dbz_map.N)
        ...
        cb = fig.colorbar(dbz_contours, ax=ax, pad=0.05, shrink=0.9, ticks=dbz_levels)
        cb.set_label("Simulated Composite Reflectivity (dBZ)", fontsize=14)
        cb.ax.set_yticklabels([f"{level}" for level in dbz_levels])
        ...
        plt.title(
            ...
            f"Simulated Composite Reflectivity (dBZ)",
            loc="left",
            fontsize=13,
        )
        plt.title(
        ...
        file_out = f"wrf_{domain}_SFC_dBZ_{fname_time}.png"
...
if __name__ == "__main__":
    ...
    # -------------------------------------------------------------------------
    path_figures = "wrf_SFC_Radar_Reflectivity"
    ...
    # -------------------------------------------------------------------------
    # Discover frames and build args list
    # -------------------------------------------------------------------------
    ...
    print("Simulated Composite dBZ plot generation complete.")
    ...
    gif_file_out = f"wrf_{domain}_SFC_Simulated_dBZ.gif"
