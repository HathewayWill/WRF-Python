#!/usr/bin/env python3
"""
Mid_Cloud_Cover_Meters.py

Plot WRF mid-level cloud fraction (meters) on a Cartopy map.

Physics (DO NOT CHANGE):
    * height_agl in meters
    * rh from WRF
    * wrf.cloudfrac with:
        low_thresh="1800", mid_thresh="3600", high_thresh="5400", meta=True
    * Mid cloud fraction = cfrac[1, :, :] * 100.0
    * Filled contours: 5–95% every 5%, Greys_r, "Mid Cloud Sky Cover (%)"
    * Title: "Mid Cloud Levels (3600 to 5400 m)"

Structure:
    * Multi-file + multi-time safe (discover_frames)
    * Moving-nest safe (geometry recomputed per frame)
    * NetCDF4 + wrf-python only (no xarray/metpy accessors)
"""
...
###############################################################################
# Mid-cloud cover (meters) plotting for one (file, time_index) frame
###############################################################################
def process_frame(args):
    """
    Process a single frame: one file and one time index.

    Physics is identical to the original script:
        * height_agl (units="m")
        * rh
        * wrf.cloudfrac with 1800/3600/5400 m thresholds
        * Mid_cfrac = cfrac[1, :, :] * 100.0
        * Filled contours using Greys_r, 5–95% every 5%.
    """
    ...
    with Dataset(ncfile_path) as ncfile:
        ...
        # Time metadata for this frame
        valid_dt = get_valid_time(ncfile, ncfile_path, time_index)
        print(f"Plotting data: {valid_dt:%Y/%m/%d %H:%M:%S} UTC")

        # -------------------------------------------------------------------------
        # Physics block (DO NOT CHANGE)
        # -------------------------------------------------------------------------
        height = wrf.getvar(ncfile, "height_agl", timeidx=time_index, units="m")
        ...
        Mid_cfrac = cfrac[1, :, :] * 100.0
        Mid_cfrac_np = to_np(Mid_cfrac)

        # -------------------------------------------------------------------------
        # Geometry / map setup (moving-nest safe)
        # -------------------------------------------------------------------------
        ...
        # -------------------------------------------------------------------------
        # Mid cloud fraction filled contours (DO NOT CHANGE)
        # -------------------------------------------------------------------------
        clevels = np.arange(5, 100, 5)  # 5–95 %, step 5

        Mid_cfrac_contour = ax.contourf(
            lons_np,
            lats_np,
            Mid_cfrac_np,
            levels=clevels,
        ...
        cbar.set_label("Mid Cloud Sky Cover (%)")
        ...
        plt.title(
            ...
            "Mid Cloud Levels (3600 to 5400 m)",
            loc="left",
            fontsize=13,
        )
        plt.title(
        ...
        # -------------------------------------------------------------------------
        # Save figure (timestamp from valid_dt for chronological sorting)
        # -------------------------------------------------------------------------
        fname_time = valid_dt.strftime("%Y%m%d%H%M%S")
        file_out = f"wrf_{domain}_Cloud_Mid_Meters_{fname_time}.png"
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 3:
        print(
            ...
            "For example:\n"
            "    Mid_Cloud_Cover_Meters.py /home/WRF/test/em_real d01\n"
        )
        sys.exit(1)
    ...
    # -------------------------------------------------------------------------
    path_figures = "wrf_Cloud_Cover_Mid_Meters"
    ...
    print("Mid Cloud Cover Plots Completed.")
    ...
    gif_file_out = f"wrf_{domain}_Mid_Cloud_Cover_Meters.gif"
