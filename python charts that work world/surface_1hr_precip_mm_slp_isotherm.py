...
from wrf import ALL_TIMES, to_np  # ALL_TIMES imported for consistency, not used here.
...
###############################################################################
# SLP + 1-hour precip plotting for one (file, time_index) frame
###############################################################################
def process_frame(args):
    ...
    Steps:
        * Read SLP and cumulative precip at this time.
        * Read cumulative precip at previous frame (if any).
        * Compute 1-hour accumulation as difference of cumulative totals (mm).
        * Build map (features, cities, gridlines).
        * Plot:
            - Smoothed SLP contours (hPa) with H/L markers.
            - 1-hour accumulated precip (mm) as filled contours.
        * Save a PNG with a valid-time-based timestamp.
    """
    (
        ncfile_path,
        time_index,
    ...
    with Dataset(ncfile_path) as ncfile:
        ...
        # -------------------------------------------------------------------------
        # Physics: SLP and cumulative precipitation (unchanged in spirit)
        # -------------------------------------------------------------------------
        ...
        # Previous frame cumulative precip for 1-hr accumulation
        if prev_ncfile_path is not None and prev_time_index is not None:
            ...
            one_hour_rain = total_rain - prev_total_rain

            earliest_dt = prev_valid_dt
            latest_dt = valid_dt
        else:
            # First frame: no previous timestep, so "1-hr" precip is zeroed
            one_hour_rain = np.zeros_like(to_np(total_rain))
            earliest_dt = valid_dt - timedelta(hours=1)
            latest_dt = valid_dt

        # 2-m temperature (retained for physics completeness, not plotted)
        ...
        # -------------------------------------------------------------------------
        # SLP contours (hPa) with H/L markers
        # -------------------------------------------------------------------------
        slp_np = to_np(slp)

        # Smooth SLP (same sigma as original)
        smooth_slp = gaussian_filter(slp_np, sigma=5.0)

        # Ensure precip field is numpy for continuity helper + plotting
        one_hour_rain_np = to_np(one_hour_rain)
        ...
        # Contour interval adjustment (preserved from original script)
        if avg_dx_km >= 9 or avg_dy_km >= 9:
            contour_interval = 4
        else:
            contour_interval = 2
        ...
        # -------------------------------------------------------------------------
        # 1-hour accumulated precipitation (inches) filled contours
        # -------------------------------------------------------------------------
        ...
        cbar.set_label("1-hour Total Precipitation (mm)", fontsize=14)
        ...
        file_out = f"wrf_{domain}_SLP_1hrPrecip_mm_{fname_time}.png"
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 3:
        print(
            ...
            "For example:\n"
            "    SFC_1hrPrecip_SLP_mm.py /home/WRF/test/em_real d01\n"
        )
        sys.exit(1)
    ...
    # -------------------------------------------------------------------------
    path_figures = "wrf_SFC_1hrPrecip_mm_figures"
    image_folder = os.path.join(path_figures, "Images")
    ...
    print("SFC SLP + 1-hour precipitation (mm) plot generation complete.")
    ...
    gif_file_out = f"wrf_{domain}_1-hour_Total_Precip_SLP_mm.gif"
