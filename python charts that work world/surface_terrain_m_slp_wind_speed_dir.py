...
and model terrain height (m) on a Cartopy map.
...
## Standard v3 feature list (keep commented options intact)
features = [
...
# Load populated places (cities) once per worker process (module-level)
...
###############################################################################
# SLP, 10-m wind, and terrain plotting for one (file, time_index) frame
###############################################################################
def process_frame(args):
    """
    Process a single frame: one file and one time index.

    Physics block (must remain identical in substance):
        ...
        * SLP levels from 870–1090 with 2 or 4 hPa spacing (based on resolution)
        * Terrain colormap, level logic, and barb brightness logic unchanged.
    """
    ...
    with Dataset(ncfile_path) as ncfile:
        ...
        # Wind components in knots (same formula)
        u10_knots = u10 * 1.94384449
        v10_knots = v10 * 1.94384449
        wind_speed_knots = np.sqrt(u10_knots**2 + v10_knots**2)  # kept even if unused

        # -------------------------------------------------------------------------
        # Grid / projection for this frame (moving-nest safe)
        # -------------------------------------------------------------------------
        ...
        # -------------------------------------------------------------------------
        # Precompute numpy fields and smoothing (physics preserved)
        # -------------------------------------------------------------------------
        ...
        # -------------------------------------------------------------------------
        # Map features, cities, and gridlines
        # -------------------------------------------------------------------------
        ...
        # -------------------------------------------------------------------------
        # SLP smoothing, H/L markers, and contours (physics preserved)
        # -------------------------------------------------------------------------
        ...
        # Contour interval logic preserved
        if avg_dx_km >= 9 or avg_dy_km >= 9:
            contour_interval = 4
        else:
            contour_interval = 2
        ...
        # -------------------------------------------------------------------------
        # Terrain colormap & levels (kept identical in logic)
        # -------------------------------------------------------------------------
        ...
        cmap_custom = colors.ListedColormap(colors_terrain)
        ...
        elif elevation_difference <= 1000:
            max_contour_levels = 25
        elif elevation_difference <= 2000:
            max_contour_levels = 50
        elif elevation_difference <= 4000:
            max_contour_levels = 75
        elif elevation_difference <= 6000:
            max_contour_levels = 100
        elif elevation_difference <= 8000:
        ...
        # Additional SLP labels (as in original)
        plt.clabel(SLP_contours, inline=True, fontsize=12, fmt="%d", colors="k")

        # -------------------------------------------------------------------------
        # Wind barbs with brightness-based coloring (logic preserved)
        # -------------------------------------------------------------------------
        ...
        # Barbs in light regions inside terrain range -> black
        ax.barbs(
        ...
        # Barbs in dark regions inside terrain range -> lightgray
        ax.barbs(
        ...
        # Barbs outside terrain level range -> black
        ax.barbs(
        ...
        # -------------------------------------------------------------------------
        # Colorbar, titles, and save
        # -------------------------------------------------------------------------
        cbar2 = fig.colorbar(
            ter_contours, ax=ax, orientation="vertical", shrink=0.8, pad=0.05
        )
        cbar2.set_label("Model Terrain Height (m)", fontsize=14)
        ...
        # Filename uses valid_dt timestamp so alphabetical sort is chronological
        fname_time = valid_dt.strftime("%Y%m%d%H%M%S")
        file_out = f"wrf_{domain}_slp_wind_ter_m_{fname_time}.png"
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 3:
        print(
            ...
            "For example: SFC_SLP_Wind_Terrain.py /home/WRF/test/em_real d01\n"
        )
        sys.exit(1)
    ...
    # Output directories
    # -------------------------------------------------------------------------
    path_figures = "wrf_SFC_Ter_m_figures"
    image_folder = os.path.join(path_figures, "Images")
    ...
    # -------------------------------------------------------------------------
    # GIF assembly from sorted PNG frames
    # -------------------------------------------------------------------------
    ...
    gif_file_out = f"wrf_{domain}_SLP_WIND_ter_m.gif"
