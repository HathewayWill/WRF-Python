#!/usr/bin/env python3
"""
SFC_Streamlines_Terrain.py

Plot WRF 10-m wind streamlines and model terrain height (ft)
on a Cartopy map.
...
from wrf import ALL_TIMES, to_np  # ALL_TIMES imported for consistency with v3 pattern
...
###############################################################################
# SFC streamlines + terrain plotting for one (file, time_index) frame
###############################################################################
def process_frame(args):
    ...
    Physics/diagnostics follow the original script:
        * 10-m wind (m/s → knots) used for streamlines.
        * Terrain height (ft) with custom terrain colormap.
        * Terrain contour level logic based on elevation range.
    """
    ncfile_path, time_index, domain, path_figures = args

    # Open the WRF file for this frame
    with Dataset(ncfile_path) as ncfile:
        # Valid time (preferred: model metadata, fallback: filename)
        valid_dt = get_valid_time(ncfile, ncfile_path, time_index)
        print(f"Plotting data: {valid_dt:%Y/%m/%d %H:%M:%S} UTC")

        # -------------------------------------------------------------------------
        # Physics block: wrf.getvar usage and math identical to original
        # -------------------------------------------------------------------------
        ...
        # Convert 10-m wind components from m/s to knots
        u10_knots = u10 * 1.94384449
        v10_knots = v10 * 1.94384449

        # (wind_speed_knots is computed in original script but unused in plotting;
        #  we preserve the calculation for scientific equivalence.)
        ...
        # -------------------------------------------------------------------------
        # Terrain colormap customization (unchanged in spirit)
        # -------------------------------------------------------------------------
        ...
        file_out = f"wrf_{domain}_streamline_ter_ft_{fname_time}.png"
...
if __name__ == "__main__":
    ...
    # -------------------------------------------------------------------------
    path_figures = "wrf_SFC_Streamlines_Ter_ft"
    ...
    print("Surface streamlines + terrain plot generation complete.")
    ...
    gif_file_out = f"wrf_{domain}_streamline_ter_ft.gif"
