#!/usr/bin/env python3
"""
SFC_Streamlines_Terrain_m.py

Plot WRF 10-m wind streamlines (knots) and model terrain height (m)
on a Cartopy map.
...
def process_frame(args):
    ...
    Physics/diagnostics follow the original script:
        * slp, U10, V10, ter from wrf.getvar (units, names unchanged).
        * 10-m wind (m/s → knots) used for streamlines.
        * Terrain height (m) with custom terrain colormap.
        * Terrain contour level logic based on elevation range (m).
    """
    ncfile_path, time_index, domain, path_figures = args

    # Open the WRF file for this frame
    with Dataset(ncfile_path) as ncfile:
        # Valid time from WRF metadata (fallback: filename)
        valid_dt = get_valid_time(ncfile, ncfile_path, time_index)
        print(f"Plotting data: {valid_dt:%Y/%m/%d %H:%M:%S} UTC")

        # -------------------------------------------------------------------------
        # Physics block (kept scientifically identical to original)
        # -------------------------------------------------------------------------
        ...
        wind_speed_knots = np.sqrt(
            u10_knots**2 + v10_knots**2
        )  # preserved, even if unused

        # -------------------------------------------------------------------------
        # Geometry: lat/lon and grid spacing (per frame)
        # -------------------------------------------------------------------------
        ...
        # Terrain levels based on elevation range (m)
        ...
        ax.streamplot(
            ...
            v10_knots_np,
            transform=crs.PlateCarree(),
            density=1.5,  # same density as original
            linewidth=1,
            color="black",
            zorder=2,
        )
        ...
        cbar.set_label("Model Terrain Height (m)", fontsize=14)
        ...
        file_out = f"wrf_{domain}_streamline_ter_m_{fname_time}.png"
...
if __name__ == "__main__":
    ...
    # -------------------------------------------------------------------------
    path_figures = "wrf_SFC_Streamlines_Ter_m"
    ...
    print("Surface streamlines + terrain (m) plot generation complete.")
    ...
    gif_file_out = f"wrf_{domain}_streamline_ter_m.gif"
