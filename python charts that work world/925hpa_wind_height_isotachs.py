...
Plot WRF 925-hPa wind barbs (knots), isotachs (knots),
and 925-hPa geopotential height (dm) on a Cartopy map.
...
###############################################################################
# 925 hPa isotach and wind plotting for one (file, time_index) frame
###############################################################################
def process_frame(args):
    ...
    Steps:
        ...
        * Interpolate to 925 hPa.
        * Convert/smooth heights, compute wind speed in knots.
        * Build map (features, cities, gridlines).
        * Plot:
            - 925 hPa heights (dm) as contours.
            - 925 hPa isotachs (knots) as filled contours.
            - 925 hPa wind barbs (knots), density adapted to grid size.
    ...
    with Dataset(ncfile_path) as ncfile:
        ...
        z = wrf.getvar(
            ncfile, "z", timeidx=time_index, units="m"
        )  # geopotential height (m)

        # -------------------------------------------------------------------------
        # Interpolate variables to 925 hPa and smooth height
        # -------------------------------------------------------------------------
        u_925 = wrf.vinterp(
            ncfile, u, "pressure", [925], extrapolate=True, squeeze=True, meta=True
        ).squeeze()
        v_925 = wrf.vinterp(
            ncfile, v, "pressure", [925], extrapolate=True, squeeze=True, meta=True
        ).squeeze()
        z_925 = wrf.vinterp(
            ncfile,
            z,
        ...
        # Convert from meters to decameters and smooth
        z_925 = gaussian_filter(to_np(z_925) / 10.0, sigma=2.0)

        # -------------------------------------------------------------------------
        # Convert wind components to knots and compute isotachs
        # -------------------------------------------------------------------------
        u_925_knots = u_925 * 1.94384449
        v_925_knots = v_925 * 1.94384449
        wind_speed_knots = np.sqrt(u_925_knots**2 + v_925_knots**2)

        u_925_knots_np = to_np(u_925_knots)
        v_925_knots_np = to_np(v_925_knots)
        ...
        # ------------------------------------------------------------------ #
        cart_proj = wrf.get_cartopy(u_925)  # native WRF map projection
        ...
        # -------------------------------------------------------------------------
        # Isotachs (wind speed) filled contours at 925 hPa
        # -------------------------------------------------------------------------
        ...
        isotachs_normalized = norm(wind_speed_knots_np)
        ...
        u_ds = u_925_knots_np[::barb_density, ::barb_density]
        v_ds = v_925_knots_np[::barb_density, ::barb_density]
...
if __name__ == "__main__":
    ...
    # -------------------------------------------------------------------------
    path_figures = "wrf_925hPa_Isotachs_Wnd_Hgt"
    ...
    print("Wind barbs and isotachs at 925 hPa plot generation complete.")
    ...
    gif_file_out = f"wrf_{domain}_925hPa_WIND_Hgt_Isotachs.gif"
