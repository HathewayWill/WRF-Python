#!/usr/bin/env python3
"""
Tropical_wrf_SFC_SSTempF.py

Plot WRF sea-level pressure (hPa), 10-m wind barbs (knots),
and sea-surface temperature (°F) masked by LANDMASK on a Cartopy map.
...
# Warning suppression
###############################################################################
# Quiet down noisy warnings (optional, but standard in v3)
...
###############################################################################
# Natural Earth features (kept commented-out options intact; order preserved)
###############################################################################
...
###############################################################################
# SLP + 10-m wind + SST(F) plotting for one (file, time_index) frame
###############################################################################
def process_frame(args):
    ...
    Physics/diagnostics preserved from original script:
        * slp, U10, V10, SST, LANDMASK via wrf.getvar
        * SST: K -> °C -> °F, masked over land
        * SLP smoothing sigma = 5.0
        * SLP contour levels 870–1090 by 4
        * SST (°F) levels 32–92 by 4
        * 'plasma' colormap for SST
        * brightness-based wind barb coloring
    """
    ...
    with Dataset(ncfile_path) as ncfile:
        ...
        # -------------------------------------------------------------------------
        # Get variables for this time (physics unchanged)
        # -------------------------------------------------------------------------
        ...
        # SST K -> °C -> °F and masked over land
        sstemp_c = temp - 273.15
        sstemp_f = (sstemp_c * (9.0 / 5.0)) + 32.0
        masked_sstemp_f = np.where(landmask == 1, np.nan, sstemp_f)

        # Wind speed (not directly plotted as contours, but preserved)
        ...
        # -------------------------------------------------------------------------
        # SLP smoothing, H/L analysis inputs (unchanged physics)
        # -------------------------------------------------------------------------
        slp_t = slp[:, :]
        masked_sstemp_f_t = masked_sstemp_f[:, :]
        ...
        # -------------------------------------------------------------------------
        # SLP smoothing, H/L analysis (unchanged physics)
        # -------------------------------------------------------------------------
        ...
        # -------------------------------------------------------------------------
        # Contours: SST (°F) + SLP (hPa)
        # -------------------------------------------------------------------------
        temp_levels = np.arange(32, 94, 4)  # °F
        ...
        # Extra SLP labels (as in original script)
        ...
        # Normalize SST to [0, 1] based on temp_levels
        norm = plt.Normalize(vmin=temp_levels[0], vmax=temp_levels[-1])
        sstemp_normalized = norm(to_np(masked_sstemp_f_t))
        ...
        outside_contour_mask = (
            (to_np(masked_sstemp_f_t) < temp_levels[0])
            | (to_np(masked_sstemp_f_t) > temp_levels[-1])
            | np.isnan(to_np(masked_sstemp_f_t))
        )
        ...
        # Barbs in SST range, light background → black
        ax.barbs(
        ...
        # Barbs in SST range, dark background → light gray
        ax.barbs(
        ...
        cbar2.set_label("Sea Surface Temperature (°F)", fontsize=14)
        ...
        plt.title(
            ...
            "Wind Barbs (knots)\n"
            "Sea Surface Temp from Input Source: (degF)",
            loc="left",
            fontsize=13,
        )
        plt.title(
        ...
        # Filename contains valid time for easy sorting
...
if __name__ == "__main__":
    ...
    # -------------------------------------------------------------------------
    path_figures = "Tropical_wrf_SFC_SSTempF_figures"
    image_folder = os.path.join(path_figures, "Images")
    ...
    print("Tropical SLP, 10-m wind, and SST (°F) plot generation complete.")
