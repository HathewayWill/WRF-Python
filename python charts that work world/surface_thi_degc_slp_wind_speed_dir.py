#!/usr/bin/env python3
"""
SFC_Temperature_Humidity_Index.py

Plot WRF sea level pressure (hPa), 10-m wind barbs (knots),
and Temperature Humidity Index (THI; original formula) on a Cartopy map.
...
###############################################################################
# SFC THI / SLP / wind plotting for one (file, time_index) frame
###############################################################################
def process_frame(args):
    """
    Process a single frame: one file and one time index.

    Physics & diagnostics block:
        ...
        * rh2_smooth    = gaussian_filter(to_np(rh2), sigma=1.0)

        * THI formula unchanged:
            THI = ((1.8 * temp_c_smooth) + 32) - (
                (0.55 - (0.0055 * rh2_smooth)) * ((1.8 * temp_c_smooth) - 26)
            )

        * smooth_slp = gaussian_filter(to_np(slp), sigma=5.0)

        * SLP levels: 870–1090 every 2 hPa
        * THI levels: 70–100 every 2
        * Wind barbs brightness logic preserved (including mask logic)
    """
    ncfile_path, time_index, domain, path_figures = args

    # Open WRF file for this frame (worker-local; no shared Dataset objects)
    with Dataset(ncfile_path) as ncfile:

        # Get valid time (canonical helper)
        valid_dt = get_valid_time(ncfile, ncfile_path, time_index)
        print(f"Plotting data: {valid_dt:%Y/%m/%d %H:%M:%S} UTC")

        # -------------------------------------------------------------------------
        # Physics: WRF variables and diagnostics (unchanged)
        # -------------------------------------------------------------------------
        slp = wrf.getvar(ncfile, "slp", timeidx=time_index)
        u10 = wrf.getvar(ncfile, "U10", timeidx=time_index)
        v10 = wrf.getvar(ncfile, "V10", timeidx=time_index)
        ...
        # Wind components (m/s → knots)
        u10_knots = u10 * 1.94384449
        v10_knots = v10 * 1.94384449

        # Keep wind speed computation identical, even if not directly plotted
        wind_speed_knots = np.sqrt(u10_knots**2 + v10_knots**2)

        # Temperature in °C (raw) and smoothing for THI computation
        temp_c_raw = temp - 273.15

        temp_c_smooth = gaussian_filter(to_np(temp_c_raw), sigma=1.0)
        rh2_smooth = gaussian_filter(to_np(rh2), sigma=1.0)

        # Temperature Humidity Index (THI) – formula unchanged
        THI = ((1.8 * temp_c_smooth) + 32) - (
            (0.55 - (0.0055 * rh2_smooth)) * ((1.8 * temp_c_smooth) - 26)
        )
        ...
        # -------------------------------------------------------------------------
        # Lat/lon, grid spacing, and extent (moving-nest safe)
        # -------------------------------------------------------------------------
        ...
        # Convert wind to numpy once (used later for barbs)
        u10_knots_np = to_np(u10_knots)
        v10_knots_np = to_np(v10_knots)

        # -------------------------------------------------------------------------
        # Dateline continuity and polar masking (v9 canonical helper)
        # Apply consistently to lat/lon and all frame fields used for plotting.
        # -------------------------------------------------------------------------
        (
            lats_np,
            lons_np,
        ...
        # -------------------------------------------------------------------------
        # High/low SLP markers (L/H) and labels
        # -------------------------------------------------------------------------
        ...
        # -------------------------------------------------------------------------
        # SLP contour levels (unchanged physics: 870–1090 every 2 hPa)
        # -------------------------------------------------------------------------
        contour_interval = 2
        SLP_start = 870
        SLP_end = 1090
        ...
        # -------------------------------------------------------------------------
        # THI contour fill (unchanged levels & colormap)
        # -------------------------------------------------------------------------
        THI_Levels = np.arange(70, 100, 2)
        ...
        THI_map = plt.matplotlib.colors.ListedColormap(color_map_rgb)
        THI_norm = plt.matplotlib.colors.BoundaryNorm(THI_Levels, THI_map.N)
        ...
        # -------------------------------------------------------------------------
        # Wind barbs with brightness-aware coloring (logic unchanged)
        # -------------------------------------------------------------------------
        ny, nx = np.shape(u10)
        desired_barbs = 15
        ...
        # Brightness for THI colormap
        color_brightness = np.dot(color_map_rgb, [0.299, 0.587, 0.114])
        brightness_threshold = 0.4

        norm = plt.Normalize(vmin=THI_Levels[0], vmax=THI_Levels[-1])
        THI_normalized = norm(THI)
        ...
        # Masking logic preserved: use T2 against THI levels
        outside_contour_mask = (
            (to_np(temp) < THI_Levels[0])
            | (to_np(temp) > THI_Levels[-1])
            | np.isnan(to_np(temp))
        )
        ...
        # Inside THI range, light background -> black barbs
        ax.barbs(
        ...
        # Inside THI range, dark background -> light gray barbs
        ax.barbs(
            lons_ds[inside_dark],
            lats_ds[inside_dark],
            u_ds[inside_dark],
        ...
        cbar2.set_label("Temperature Humidity Index (°C)", fontsize=14)
        ...
        file_out = f"wrf_{domain}_slp_wind_THI_{fname_time}.png"
...
if __name__ == "__main__":
    ...
    # -------------------------------------------------------------------------
    path_figures = "wrf_SFC_THI_degC_figures"
    ...
    # -------------------------------------------------------------------------
    # Process frames in parallel using ProcessPoolExecutor (required pattern)
    # -------------------------------------------------------------------------
    ...
    gif_file_out = f"wrf_{domain}_SLP_WIND_THI.gif"
