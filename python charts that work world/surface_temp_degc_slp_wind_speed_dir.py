#!/usr/bin/env python3
"""
SFC_SLP_WIND_TEMPC.py

Plot WRF sea level pressure (hPa), 10-m wind barbs (knots),
and 2-m temperature (°C) with 0°C isotherm on a Cartopy map.
...
###############################################################################
# SLP + surface wind + 2m Temp plotting for one (file, time_index) frame
###############################################################################
def process_frame(args):
    ...
    Physics/diagnostics preserved from original:
        * slp, U10, V10, T2 from wrf.getvar (names/units unchanged).
        * T2 smoothed with Gaussian sigma=1.0, converted to °C.
        * slp smoothed with Gaussian sigma=5.0.
        * SLP contour start/stop/interval unchanged.
        * Temp levels, colormap, and 0°C isotherm unchanged.
        * Wind barbs colored based on background brightness, same logic.
    """
    ...
    with Dataset(ncfile_path) as ncfile:
        ...
        # -------------------------------------------------------------------------
        # Physics block (unchanged math / parameters)
        # -------------------------------------------------------------------------
        ...
        # Smooth T2 and convert from K to °C
        temp_c = gaussian_filter(to_np(temp), sigma=1.0) - 273.15

        # Wind speed in knots (computed but not used directly in plotting)
        wind_speed_knots = np.sqrt(u10_knots**2 + v10_knots**2)

        # -------------------------------------------------------------------------
        # Geometry: lat/lon & grid spacing per frame
        # -------------------------------------------------------------------------
        ...
        # -------------------------------------------------------------------------
        # H/L markers and labels (unchanged logic)
        # -------------------------------------------------------------------------
        ...
        # -------------------------------------------------------------------------
        # Temperature levels and colormap (unchanged)
        # -------------------------------------------------------------------------
        ...
        temp_map = mcolors.ListedColormap(color_map_rgb)
        temp_norm = mcolors.BoundaryNorm(temp_levels, temp_map.N)

        # Contour interval adjustment (unchanged)
        if avg_dx_km >= 9 or avg_dy_km >= 9:
            contour_interval = 4
        else:
            contour_interval = 2

        # SLP contour levels (unchanged)
        SLP_start = 870
        SLP_end = 1090
        ...
        # -------------------------------------------------------------------------
        # Wind barbs with brightness-aware coloring (unchanged logic)
        # -------------------------------------------------------------------------
        ...
        temp_normalized = norm_temp(temp_c)
        ...
        outside_contour_mask = (
            (to_np(temp) < temp_levels[0])
            | (to_np(temp) > temp_levels[-1])
            | np.isnan(to_np(temp))
        )
        ...
        # Barbs inside contour range, light background → black
        ax.barbs(
        ...
        # Barbs inside contour range, dark background → light gray
        ax.barbs(
        ...
        cbar2.set_label("Surface Temperature (°C)", fontsize=14)
        ...
        file_out = f"wrf_{domain}_slp_wind_temp_{fname_time}.png"
...
if __name__ == "__main__":
    ...
    print("SFC SLP/Wind/Temp plot generation complete.")
