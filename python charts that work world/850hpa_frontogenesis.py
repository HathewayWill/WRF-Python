#!/usr/bin/env python3
"""
850hPa_Frontogenesis_Temp_Height_Wind_multicore_golden.py

WRF Golden Playbook v3

Plots:
    * 850-hPa Frontogenesis (K / 100 km / 3 h)
    * 850-hPa Temperature (°C)
    * 850-hPa Geopotential Height (dm)
    * 850-hPa Wind Barbs (kt)

Supports:
    * Multi-file / multi-time WRF output
    * Moving nests
...
###############################################################################
# 850 hPa frontogenesis, temp, heights, and wind plotting for one frame
###############################################################################
def process_frame(args):
    ...
    with Dataset(ncfile_path) as ncfile:
        ...
        # -------------------------------------------------------------------------
        # Vertical interpolation (IMMUTABLE PHYSICS)
        # -------------------------------------------------------------------------
        t850 = wrf.vinterp(
        ...
        z850 = wrf.vinterp(
            ncfile, z, "pressure", [850], extrapolate=True, squeeze=True, meta=True
        )

        # ---- HARD DIMENSION NORMALIZATION (Golden requirement) ----
        t850 = t850.squeeze()
        u850 = u850.squeeze()
        v850 = v850.squeeze()
        z850 = z850.squeeze()
        ...
        # -------------------------------------------------------------------------
        # Unit handling (strip masked arrays FIRST)
        # -------------------------------------------------------------------------
        tmpk = np.asarray(to_np(t850), dtype=np.float32) * units.kelvin
        tmpc = tmpk.to("degC")

        uwnd = np.asarray(to_np(u850), dtype=np.float32) * units("m/s")
        vwnd = np.asarray(to_np(v850), dtype=np.float32) * units("m/s")

        hgt = gaussian_filter(to_np(z850), sigma=1.5)

        # -------------------------------------------------------------------------
        # Grid geometry (per frame, moving-nest safe)
        # -------------------------------------------------------------------------
        ...
        # -------------------------------------------------------------------------
        # Frontogenesis (Cartesian, MetPy-safe)
        # -------------------------------------------------------------------------
        theta = mpcalc.potential_temperature(850 * units.hPa, tmpk)

        theta_i = theta[1:-1, 1:-1]
        uwnd_i = uwnd[1:-1, 1:-1]
        vwnd_i = vwnd[1:-1, 1:-1]

        # Use scalar grid spacing (Golden canonical)
        dx_s = (avg_dx_km * units.kilometer).to("meter")
        dy_s = (avg_dy_km * units.kilometer).to("meter")

        fronto = mpcalc.frontogenesis(
            theta_i,
            uwnd_i,
            vwnd_i,
            dx=dx_s,
        ...
        # --- Gaussian smooth frontogenesis (sigma in GRID POINTS) ---
        sigma = 2.0
        fronto_mag_sm = gaussian_filter(
            fronto.magnitude.astype(np.float32), sigma=sigma, mode="nearest"
        )

        # keep as Quantity (optional but nice)
        fronto = fronto_mag_sm * fronto.units
        fronto = fronto.squeeze()
        # Convert to K / 100 km / 3 h
        fronto = fronto * (1000 * 100 * 3600 * 3)

        # Grid for plotting (interior)
        lons_i = lons_np[1:-1, 1:-1]
        lats_i = lats_np[1:-1, 1:-1]
        ...
        (
            ...
            lats_i,
            lons_i,
            np.squeeze(fronto.magnitude),
        )

        # -------------------------------------------------------------------------
        # Figure and projection (Golden resolution)
        # -------------------------------------------------------------------------
        proj = wrf.get_cartopy(t850)
        ...
        # -------------------------------------------------------------------------
        # 850 hPa height contours (dm) — Golden standard
        # -------------------------------------------------------------------------
        z850_start = 60  # dm
        z850_end = 200  # dm
        ...
        skip = max(int(avg_dx_km // 3), 5)
        ax.barbs(
            lons_np[::skip, ::skip],
            lats_np[::skip, ::skip],
            (u_mag_ms * units("m/s")).to("kt").magnitude[::skip, ::skip],
            (v_mag_ms * units("m/s")).to("kt").magnitude[::skip, ::skip],
            transform=crs.PlateCarree(),
        )
        ...
        plt.title(
            ...
            f"Frontogenesis at 850 hPa (K / 100 km / 3 h)\n"
            f"Temperature at 850 hPa (°C)\n"
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 3:
        print("Usage: python 850hPa_frontogenesis.py <path_wrf> <domain>")
        sys.exit(1)

    path_wrf = sys.argv[1]
    domain = sys.argv[2]

    path_figures = "wrf_850hPa_Frontogenesis"
    ...
    if not png_files:
        print("No PNG files found for GIF generation.")
        sys.exit(0)
    ...
    gif_path = os.path.join(
        animation_folder,
        f"wrf_{domain}_850hPa_Frontogenesis.gif",
    )
