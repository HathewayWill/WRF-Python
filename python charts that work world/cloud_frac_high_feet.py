...
Plot WRF high cloud fraction (% sky cover above 17,700 ft)
on a Cartopy map.
...
It does NOT assume the domain is static:
    * For each timestep, lat/lon, grid spacing, extent, and cities
      are recomputed from the WRF fields (safe for moving nests).
"""
...
###############################################################################
# High cloud cover plotting for one (file, time_index) frame
###############################################################################
def process_frame(args):
    """
    Process a single frame: one file and one time index.

    Physics kept identical to original script:
        - height_agl (ft)
        - rh
        - wrf.cloudfrac with the same thresholds and options
        - High_cfrac = cfrac[2, :, :] * 100
        - Same contour levels and colormap for high cloud fraction.
    """
    ...
    with Dataset(ncfile_path) as ncfile:
        ...
        # -------------------------------------------------------------------------
        # Physics: get variables and compute high cloud fraction (UNCHANGED)
        # -------------------------------------------------------------------------
        ...
        # High cloud fraction: 3rd layer, convert to %
        High_cfrac = cfrac[2, :, :] * 100.0
        ...
        High_cfrac_np = to_np(High_cfrac)
        ...
        # -------------------------------------------------------------------------
        # Plot high cloud fraction (physics: levels & cmap unchanged)
        # -------------------------------------------------------------------------
        clevels = np.arange(5, 100, 5)

        High_cfrac_contour = ax.contourf(
            lons_np,
            lats_np,
            High_cfrac_np,
            levels=clevels,
        ...
        cbar.set_label("High Cloud Sky Cover (%)")

        # -------------------------------------------------------------------------
        # Titles and saving figure
        # -------------------------------------------------------------------------
        plt.title(
            f"Weather Research and Forecasting Model\n"
            f"Average Grid Spacing:{avg_dx_km}x{avg_dy_km}km\n"
            f"High Cloud Levels (above 17,700ft)",
            loc="left",
            fontsize=13,
        )
        plt.title(
        ...
        # Filename uses valid time → chronological sort = alphabetical sort
        fname_time = valid_dt.strftime("%Y%m%d%H%M%S")
        file_out = f"wrf_{domain}_Cloud_High_{fname_time}.png"
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 3:
        print(
            ...
            "For example:\n"
            "    High_Cloud_Cover.py /home/WRF/test/em_real d01\n"
        )
        sys.exit(1)
    ...
    # -------------------------------------------------------------------------
    path_figures = "wrf_Cloud_Cover_High_Feet"
    ...
    print("High Cloud Cover Plots Completed.")

    # -------------------------------------------------------------------------
    # Build animated GIF from the sorted PNG files (v3 standard)
    # -------------------------------------------------------------------------
    ...
    gif_file_out = f"wrf_{domain}_High_Cloud_Cover_feet.gif"
