#!/usr/bin/env python3
"""
Low_Cloud_Cover_Meters.py

Plot WRF low-level cloud fraction (%) based on height AGL (meters)
using wrf.cloudfrac and show the result on a Cartopy map.

Low cloud definition (unchanged from original script):
    low_thresh  = 1800 m
    mid_thresh  = 3600 m
    high_thresh = 5400 m
...
###############################################################################
# Low cloud fraction plotting for one (file, time_index) frame
###############################################################################
def process_frame(args):
    """
    Process a single frame: one file and one time index.

    Physics/diagnostics (unchanged from original):
        height = wrf.getvar(ncfile, "height_agl", units="m")
        rh     = wrf.getvar(ncfile, "rh")
        cfrac  = wrf.cloudfrac(
                     vert=height,
                     relh=rh,
                     vert_inc_w_height=1,
    ...
    with Dataset(ncfile_path) as ncfile:
        ...
        # -------------------------------------------------------------------------
        # Physics block (unchanged computation)
        # -------------------------------------------------------------------------
        ...
        # *** IMPORTANT: use rh for lat/lon and projection, like the original ***
        # -------------------------------------------------------------------------
        ...
        (
            ...
            avg_dy_km,
            extent_adjustment,
            label_adjustment,  # reserved for future label offsets
        ...
        # -------------------------------------------------------------------------
        # Low cloud fraction fill (same levels/colormap as original)
        # -------------------------------------------------------------------------
        ...
        plt.title(
            ...
            f"Low Cloud Levels (1800m to 3600m)",
            loc="left",
            fontsize=13,
        )
        plt.title(
        ...
        file_out = f"wrf_{domain}_Cloud_Low_Meters_{fname_time}.png"
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 3:
        print(
            ...
            "For example:\n"
            "    Low_Cloud_Cover_Meters_multicore.py "
    ...
    # -------------------------------------------------------------------------
    path_figures = "wrf_Cloud_Cover_Low_Meters"
    ...
    gif_file_out = f"wrf_{domain}_Low_Cloud_Cover_Meters.gif"
