...
and surface Heat Index (°C) on a Cartopy map.

This v3 script can handle:
    * Multiple wrfout_<domain>* files, each with one or more timesteps.
    * A single wrfout file containing many timesteps.

For each frame (file, time_index):
    * Lat/lon, grid spacing, and extent are recomputed from the fields.
    * Works for both static nests and moving/vortex-following nests.
"""
...
# Warning suppression
###############################################################################
# Quiet down noisy warnings (optional, but standard)
...
def calculate_heat_index(T, RH):
    """
    Calculate Heat Index (°C) given temperature (°F) and relative humidity (%).
    Handles NumPy arrays correctly.

    Parameters:
        T (float or np.array): Temperature in degrees Fahrenheit.
        RH (float or np.array): Relative humidity in percent.

    Returns:
        np.array or float: Heat Index in degrees Celsius.
    """
...
###############################################################################
# Heat Index + SLP + wind plotting for one (file, time_index) frame
###############################################################################
def process_frame(args):
    """
    Process a single frame: one file and one time index.

    Physics block is kept scientifically identical to the original script:
        * slp, U10, V10, T2, rh2 access via wrf.getvar.
        * Heat Index formula and smoothing parameters.
        * SLP contour ranges and intervals.
        * Heat Index levels and colormap.
        * Wind barb brightness logic.
    """
    ...
    with Dataset(ncfile_path) as ncfile:
        ...
        # -------------------------------------------------------------------------
        # Get WRF variables at this time index (physics unchanged)
        # -------------------------------------------------------------------------
        ...
        # Temperature to °F
        temp_f_raw = (temp - 273.15) * 9 / 5 + 32.0

        # Smooth temperature and RH for HI calculation (sigma unchanged)
        temp_f_smooth = gaussian_filter(temp_f_raw, sigma=1.0)
        rh2_smooth = gaussian_filter(rh2, sigma=1.0)

        # Heat Index in °C (function unchanged)
        heat_index = calculate_heat_index(temp_f_smooth, rh2_smooth)

        # Wind speed in knots (not directly plotted, but preserved)
        wind_speed_knots = np.sqrt(u10_knots**2 + v10_knots**2)

        # -------------------------------------------------------------------------
        # Lat/lon, grid spacing, projection (moving-nest safe)
        # -------------------------------------------------------------------------
        ...
        # -------------------------------------------------------------------------
        slp_np_for_smoothing = to_np(slp)
        temp_np_for_mask = to_np(temp)

        smooth_slp = gaussian_filter(slp_np_for_smoothing, sigma=5.0)
        ...
        # -------------------------------------------------------------------------
        # Base map features + cities + gridlines
        # -------------------------------------------------------------------------
        ...
        # -------------------------------------------------------------------------
        # Smooth SLP and mark H/L centers (sigma unchanged)
        # -------------------------------------------------------------------------
        ...
        # -------------------------------------------------------------------------
        # SLP contours and Heat Index filled contours (levels unchanged)
        # -------------------------------------------------------------------------
        HI_Levels = np.arange(23, 52, 3)
        ...
        # -------------------------------------------------------------------------
        # Wind barbs with brightness-adaptive coloring (logic unchanged)
        # -------------------------------------------------------------------------
        ...
        outside_contour_mask = (
            (temp_np_for_mask < HI_Levels[0])
            | (temp_np_for_mask > HI_Levels[-1])
            | np.isnan(temp_np_for_mask)
        )
        ...
        # Inside HI range, light background → black barbs
        ax.barbs(
        ...
        # Inside HI range, dark background → light gray barbs
        ax.barbs(
        ...
        cbar2.set_label("Heat Index (°C)", fontsize=14)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 3:
        print(
            ...
            "For example:\n"
            "    SFC_HeatIndex.py /home/WRF/test/em_real d01\n"
        )
        sys.exit(1)
    ...
    # -------------------------------------------------------------------------
    path_figures = "wrf_SFC_HeatIndex_degC_figures"
    ...
    print("SFC Heat Index plots complete.")
