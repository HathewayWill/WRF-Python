#!/usr/bin/env python3
"""
SFC_24hrSnow_Inch.py

Plot WRF 24-hour accumulated snow depth (inches) on a Cartopy map.
...
###############################################################################
# 24-hr snow plotting for one (file, time_index) frame
###############################################################################
def process_frame(args):
    ...
    Physics / diagnostics preserved from the original script:
        ...
        * total_snow (in) = snow * 39.3700787402
        * 24-hr accumulation = total_snow - previous_total_snow
          from the frame 24 steps back (also in inches).
    ...
    with Dataset(ncfile_path) as ncfile:
        ...
        # -------------------------------------------------------------------------
        # Physics: get variables with explicit time index
        # -------------------------------------------------------------------------
        slp = wrf.getvar(ncfile, "slp", timeidx=time_index)
        snow = wrf.getvar(ncfile, "SNOWH", timeidx=time_index)

        # Cumulative snow depth (meters -> inches)
        total_snow = snow * 39.3700787402

        # 24-hour accumulated snow using the frame 24 steps back
        if prev24_ncfile_path is not None and prev24_time_index is not None:
            with Dataset(prev24_ncfile_path) as prev24_ncfile:
                prev24_snow = wrf.getvar(
                    prev24_ncfile, "SNOWH", timeidx=prev24_time_index
                )
                prev24_total_snow = prev24_snow * 39.3700787402
        ...
        # -------------------------------------------------------------------------
        # 0 to 42 inch per 24-hour colorbar (unchanged)
        # -------------------------------------------------------------------------
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 3:
        print(
            ...
            "For example: SFC_24hrSnow_Inch_multicore_v3.py "
    ...
    # -------------------------------------------------------------------------
    path_figures = "wrf_SFC_24hrSNOW_Inch_figures"
    image_folder = os.path.join(path_figures, "Images")
    ...
    print("Surface 24-hour snow (inches) plot generation complete.")
    ...
    # Preserve original behavior: only frames with >=24h accumulation
    if len(images) < 24:
        sys.exit(
            "The wrf run is less than 24 hours and cannot make 24hr total accumulation."
        )

    images = images[23:]  # drop first 23 frames, as in original script

    gif_file_out = f"wrf_{domain}_24-hour_Total_Snow_Inch.gif"
    ...
    print(f"SFC 24hr Snow Depth Inches GIF complete: {gif_path}")
