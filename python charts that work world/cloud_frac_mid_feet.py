#!/usr/bin/env python3
"""
CloudFrac_Mid_Feet.py

Plot WRF mid-level cloud fraction (%) on a Cartopy map.

Mid cloud definition (physics preserved from original script):
    - height_agl (ft)
    - wrf.cloudfrac with:
        low_thresh  = "5900"
        mid_thresh  = "11800"
        high_thresh = "17700"
    → Mid cloud = cfrac[1, :, :] * 100.0  (11,800 ft – 17,700 ft)
...
It does NOT assume the domain is static:
    * For each timestep, lat/lon, grid spacing, and extent are
      recomputed from the WRF fields. This works for static and
...
from wrf import ALL_TIMES, to_np  # ALL_TIMES kept for consistency, though unused
...
###############################################################################
# Mid cloud fraction plotting for one (file, time_index) frame
###############################################################################
def process_frame(args):
    """
    Process a single frame: one file and one time index.

    Physics block is identical to original:
        height_agl (ft), rh, wrf.cloudfrac(...),
    ...
    with Dataset(ncfile_path) as ncfile:
        ...
        # Valid time from metadata (fallback: filename)
        ...
        # Lat/lon from a field that carries coordinates (rh)
        lats, lons = wrf.latlon_coords(rh)

        # Grid spacing, extent, label offsets (moving-nest safe)
        (
            lats_np,
            lons_np,
            avg_dx_km,
        ...
        # Native WRF projection for this domain/time
        ...
        # -------------------------------------------------------------------------
        # Mid cloud fraction filled contours
        # -------------------------------------------------------------------------
        clevels = np.arange(5, 100, 5)  # 5–95% in 5% increments (matches original)
        ...
        plt.title(
            ...
            "Mid Cloud Levels (11,800 ft to 17,700 ft)",
            loc="left",
            fontsize=13,
        )
        plt.title(
        ...
        # -------------------------------------------------------------------------
        # Save PNG (timestamp from valid_dt for GIF sorting)
        # -------------------------------------------------------------------------
        fname_time = valid_dt.strftime("%Y%m%d%H%M%S")
        file_out = f"wrf_{domain}_Cloud_Mid_{fname_time}.png"
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 3:
        print(
            ...
            "For example:\n"
            "    CloudFrac_Mid_Feet_multicore_v3.py /home/WRF/test/em_real d01\n"
        )
        sys.exit(1)
    ...
    # -------------------------------------------------------------------------
    path_figures = "wrf_Cloud_Cover_Mid_Feet"
    ...
    # -------------------------------------------------------------------------
    # Discover (file, time_index) frames
    # -------------------------------------------------------------------------
    ...
    # Filenames include YYYYMMDDHHMMSS → alphabetical sort is chronological
    ...
    gif_file_out = f"wrf_{domain}_Mid_Cloud_Cover_Feet.gif"
