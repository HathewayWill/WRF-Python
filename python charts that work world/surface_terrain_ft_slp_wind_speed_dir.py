#!/usr/bin/env python3
"""
SFC_SLP_Wind_Terrain.py

Plot WRF sea level pressure (hPa), 10-m wind barbs (knots),
and model terrain height (ft) on a Cartopy map.

Supports:
    * Multiple wrfout_<domain>* files with one or more timesteps.
    * A single wrfout file containing many timesteps.
    * Mixed layouts.

Geometry (lat/lon, grid spacing, extent, cities) is recomputed
per frame → safe for static and moving/vortex-following nests.
"""
...
###############################################################################
# Preserved original helper docstrings / comments (verbatim text preserved)
###############################################################################
"""
...
###############################################################################
# SLP + 10m wind + terrain plotting for one (file, time_index) frame
###############################################################################
def process_frame(args):
    ...
    Physics/diagnostics preserved from original script:
        * slp, U10, V10, ter (ft) via wrf.getvar
        * SLP smoothing (Gaussian, sigma=5.0)
        * SLP contour range & interval
        * Terrain colormap modification and level selection
        * Wind speed in knots and barb brightness logic
    """
    ...
    with Dataset(ncfile_path) as ncfile:
        ...
        ter = wrf.getvar(ncfile, "ter", timeidx=time_index, units="ft")

        u10_knots = u10 * 1.94384449
        v10_knots = v10 * 1.94384449

        # Wind speed in knots (kept although not explicitly contoured)
        ...
        # -------------------------------------------------------------------------
        # Precompute numpy fields, then apply dateline continuity / polar masking
        # (Only changes ordering/masking when needed; preserves physics values.)
        # -------------------------------------------------------------------------
        ...
        # Land and standard features
        ax.add_feature(cfeature.LAND, facecolor=cfeature.COLORS["land"])
        for feature in features:
            add_feature(ax, *feature)

        # Cities for this frame
        ...
        # -------------------------------------------------------------------------
        # SLP contours (physics: range & interval unchanged)
        # -------------------------------------------------------------------------
        contour_interval = 2
        SLP_start = 870
        SLP_end = 1090
        ...
        # -------------------------------------------------------------------------
        # Terrain colormap modifications (unchanged logic)
        # -------------------------------------------------------------------------
        cmap_terrain = plt.cm.get_cmap("terrain")
        colors_terrain = cmap_terrain(np.linspace(0, 1, 256))

        transition_index = 100
        for i in range(transition_index):
            alpha = i / transition_index
            colors_terrain[i] = (
                alpha * colors_terrain[transition_index]
                + (1 - alpha) * colors_terrain[i]
            )

        cmap_custom = mcolors.ListedColormap(colors_terrain)

        # Brightness for barb contrast
        rgb_values = cmap_custom(np.linspace(0, 1, cmap_custom.N))[:, :3]

        # Terrain levels as in original logic
        ter_min = np.min(ter_np)
        ter_max = np.max(ter_np)
        elevation_difference = ter_max - ter_min

        if elevation_difference <= 1600:
            max_contour_levels = 10
        elif elevation_difference <= 3200:
            max_contour_levels = 25
        elif elevation_difference <= 6500:
            max_contour_levels = 50
        elif elevation_difference <= 13000:
            max_contour_levels = 75
        elif elevation_difference <= 18000:
            max_contour_levels = 100
        elif elevation_difference <= 25000:
            max_contour_levels = 150
        else:
        ...
        # Label SLP contours (same as original)
        plt.clabel(SLP_contours, inline=True, fontsize=12, fmt="%d", colors="k")

        # -------------------------------------------------------------------------
        # Wind barbs with brightness-based color
        # -------------------------------------------------------------------------
        ...
        # Color brightness from terrain colormap
        color_brightness = np.dot(rgb_values, [0.299, 0.587, 0.114])
        brightness_threshold = 0.4

        norm = plt.Normalize(vmin=ter_levels[0], vmax=ter_levels[-1])
        ter_normalized = norm(ter_np)

        brightness_map = np.interp(
            ter_normalized, np.linspace(0, 1, len(color_brightness)), color_brightness
        )
        ...
        outside_contour_mask = (
            (ter_np < ter_levels[0]) | (ter_np > ter_levels[-1]) | np.isnan(ter_np)
        )
        ...
        # Inside terrain range, light background → black barbs
        ax.barbs(
        ...
        # Inside terrain range, dark background → light gray barbs
        ax.barbs(
            lons_ds[inside_dark],
            lats_ds[inside_dark],
            u_ds[inside_dark],
        ...
        # Outside terrain contour range → black barbs
        ax.barbs(
        ...
        # -------------------------------------------------------------------------
        cbar2 = fig.colorbar(
            ter_contours, ax=ax, orientation="vertical", shrink=0.8, pad=0.05
        )
        cbar2.set_label("Model Terrain Height (ft)", fontsize=14)
        ...
        # Filename with embedded timestamp for GIF sorting
        fname_time = valid_dt.strftime("%Y%m%d%H%M%S")
        file_out = f"wrf_{domain}_slp_wind_ter_ft_{fname_time}.png"
...
if __name__ == "__main__":
    ...
    # -------------------------------------------------------------------------
    path_figures = "wrf_SFC_Ter_ft_figures"
    ...
    # -------------------------------------------------------------------------
    # Build list of frames and process in parallel
    # -------------------------------------------------------------------------
    ...
    print("Surface SLP, wind, and terrain plots complete.")
    ...
    gif_file_out = f"wrf_{domain}_SLP_WIND_ter_ft.gif"
