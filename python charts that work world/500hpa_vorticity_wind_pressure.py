...
Plot WRF 500-hPa wind barbs (knots), absolute vorticity (s⁻¹),
and 500-hPa geopotential height (dm) on a Cartopy map.
...
###############################################################################
# 500 hPa vorticity + wind plotting for one (file, time_index) frame
###############################################################################
def process_frame(args):
    ...
    # Get base WRF variables for this time index
    ...
    avo = wrf.getvar(ncfile, "avo", timeidx=time_index)  # absolute vorticity

    # Interpolate to 500 hPa
    u_500 = wrf.interplevel(u, p, 500)
    v_500 = wrf.interplevel(v, p, 500)
    z_500 = wrf.interplevel(z, p, 500)
    avo_500 = wrf.interplevel(avo, p, 500)

    # Smooth heights and vorticity
    z_500_smooth = gaussian_filter(to_np(z_500), sigma=2.0)
    avo_500_smooth = gaussian_filter(to_np(avo_500), sigma=1.0)
    ...
    # Lat/lon and grid spacing (static or moving nests)
    ...
    # Map extent padded slightly beyond domain for nicer framing
    ax.set_extent(
        [
    ...
    # Add land & map features
    ax.add_feature(cfeature.LAND, facecolor=cfeature.COLORS["land"])
    for feature in features:
        add_feature(ax, *feature)

    # Plot cities and gridlines
    ...
    # Vorticity color fill
    vort_levels = np.array([-30, -25, -20, -15, -10, -5, 0, 5, 10, 15, 20, 25, 30])
    ...
    vort_map = plt.matplotlib.colors.ListedColormap(color_map_rgb)
    vort_norm = plt.matplotlib.colors.BoundaryNorm(vort_levels, vort_map.N)

    vort_contourf = ax.contourf(
        lons_np,
        lats_np,
        avo_500_smooth,
        levels=vort_levels,
    ...
    cbar.set_label("Vorticity (s⁻¹)")
    cbar.ax.set_yticklabels([f"{level:.0f}" for level in vort_levels])
    ...
    # Background brightness based on vorticity colormap
    color_brightness = np.dot(color_map_rgb, [0.299, 0.587, 0.114])
    brightness_threshold = 0.4

    norm_vort = plt.Normalize(vmin=vort_levels[0], vmax=vort_levels[-1])
    vort_normalized = norm_vort(avo_500_smooth)
    ...
    outside_contour_mask = (
        (avo_500_smooth < vort_levels[0])
        | (avo_500_smooth > vort_levels[-1])
        | np.isnan(avo_500_smooth)
    )
    inside_contour_mask = ~outside_contour_mask

    inside_light = (inside_contour_mask & light_region)[::barb_density, ::barb_density]
    inside_dark = (inside_contour_mask & dark_region)[::barb_density, ::barb_density]
    ...
    u_ds = u_500_knots_np[::barb_density, ::barb_density]
    v_ds = v_500_knots_np[::barb_density, ::barb_density]

    # Inside vorticity range, light background
    ax.barbs(
    ...
    # Filename with full timestamp for robust sorting
    fname_time = valid_dt.strftime("%Y%m%d%H%M%S")
    file_out = f"wrf_{domain}_wind_500hPa_{fname_time}.png"
...
if __name__ == "__main__":
    ...
    # Path to WRF output and requested domain (d01, d02, ...)
    path_wrf = sys.argv[1]
    domain = sys.argv[2]

    # Output folders for static images and GIF animation
    path_figures = "wrf_500hPa_Vort_Wind_Hgt"
    ...
    # Collect all wrfout files for the chosen domain
    ...
    # Build full list of (file, time_index) frames
    ...
    print("500 hPa plot generation complete.")

    # Gather all generated PNGs and sort them by filename (timestamp)
    ...
    gif_file_out = f"wrf_{domain}_500hPa_WIND_Hgt_Vort.gif"
