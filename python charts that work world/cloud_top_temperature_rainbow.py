#!/usr/bin/env python3
"""
Cloud_Top_Temp_color.py

Plot WRF cloud-top temperature (°C) on a Cartopy map using a color (rainbow)
shading.
...
###############################################################################
# Cloud-top temperature plotting for one (file, time_index) frame
###############################################################################
def process_frame(args):
    ...
    Steps:
        * Read WRF cloud-top temperature (ctt) at this time.
        * Compute grid spacing and map extent.
        * Build map (features, cities, gridlines).
        * Plot cloud-top temperature as filled contours.
    ...
    with Dataset(ncfile_path) as ncfile:
        ...
        # -------------------------------------------------------------------------
        # Physics block (unchanged in spirit)
        # -------------------------------------------------------------------------
        # Cloud-top temperature (°C)
        ctt = wrf.getvar(ncfile, "ctt", timeidx=time_index)

        # Latitude/longitude
        lats, lons = wrf.latlon_coords(ctt)

        # Grid spacing and extent/label adjustments (moving-nest safe)
        (
            lats_np,
            lons_np,
            avg_dx_km,
        ...
        # -------------------------------------------------------------------------
        # Cloud-top temperature filled contours (rainbow color version)
        # -------------------------------------------------------------------------
        contour_levels = [-100, -80, -70, -60, -50, -40, -30, -20, -10, 0, 10]

        ctt_contours = ax.contourf(
            lons_np,
            lats_np,
            ctt_np,
        ...
        # Colorbar (same quantity/units as original)
        cb_ctt = fig.colorbar(ctt_contours, ax=ax)
        cb_ctt.ax.tick_params(labelsize=5)
        cb_ctt.set_label("Temperature °C")
        ...
        file_out = f"wrf_{domain}_Cloud_Top_Temp_color_{fname_time}.png"
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 3:
        print(
            ...
            "For example:\n"
            "    Cloud_Top_Temp_color.py /home/WRF/test/em_real d01\n"
        )
        sys.exit(1)
    ...
    # -------------------------------------------------------------------------
    path_figures = "wrf_Cloud_Top_Temp_color"
    ...
    print("Cloud Top Temp Plots Completed.")
    ...
    gif_file_out = f"wrf_{domain}_Cloud_Top_Temp_color.gif"
