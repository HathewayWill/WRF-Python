...
Plot WRF 24-hour accumulated snow depth (mm) on a Cartopy map.
...
def process_frame(args):
    ...
    Physics / diagnostics preserved from the original script:
        ...
        * snow = wrf.getvar(ncfile, "SNOWH", timeidx=time_index) * 1000  # m→mm
        * 24-hr accumulation = total_snow - previous_total_snow
          from the frame 24 steps back (also in mm).
        * Snow_levels array and RGB colormap unchanged.
        * SLP smoothing with sigma=5.0 kept (even though SLP is not contoured).
    """
    (
        ncfile_path,
        time_index,
    ...
    with Dataset(ncfile_path) as ncfile:
        ...
        # Valid time from metadata or filename
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 3:
        print(
            ...
            "For example: SFC_24hrSnow_mm_multicore_v3.py "
    ...
    # -------------------------------------------------------------------------
    path_figures = "wrf_SFC_24hrSNOW_mm_figures"
    image_folder = os.path.join(path_figures, "Images")
    ...
    print("Surface 24-hour snow (mm) plot generation complete.")
