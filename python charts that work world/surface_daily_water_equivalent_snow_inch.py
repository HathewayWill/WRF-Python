...
Daily (00–00 UTC and 12–12 UTC) accumulated snowfall (inches) based on SNOWH (m)
...
###############################################################################
# Frame processing: one daily 00Z–00Z or 12Z–12Z snowfall accumulation
###############################################################################
def process_frame(args):
    """
    Process a single daily frame (same-cycle snowfall accumulation).

    This can be a 00Z–00Z frame or a 12Z–12Z frame, depending on how frames
    are paired in the calling logic.

    args:
        ncfile_path      : path to current cycle WRF file
        time_index       : time index for current cycle in ncfile_path
        prev_ncfile_path : path to previous same-cycle WRF file
        prev_time_index  : time index for previous same-cycle in prev_ncfile_path
    ...
    # Need a previous frame at the same cycle time to compute a daily accumulation
    ...
    with Dataset(ncfile_path) as ncfile, Dataset(prev_ncfile_path) as prev_ncfile:
        ...
        # -------------------------------------------------------------------------
        # Compute daily accumulated snowfall between two same-cycle times (inches)
        # -------------------------------------------------------------------------
        daily_snow_00z = total_snow - prev_total_snow
        ...
        # -------------------------------------------------------------------------
        # Daily snow (inches) filled contours: levels & colormap preserved
        # -------------------------------------------------------------------------
...
if __name__ == "__main__":
    ...
    # -------------------------------------------------------------------------
    # Output directories root (cycle-specific subdirectories will be created)
    # -------------------------------------------------------------------------
    path_figures_root = "wrf_SFC_DailySnow_Inch_figures"
    ...
    for cycle_hour, cycle_label, discover_func in cycles:
        ...
        # -------------------------------------------------------------------------
        # Process frames in parallel (only frames with a previous same-cycle
        # timestep produce PNGs)
        # -------------------------------------------------------------------------
        ...
        print(
            "SFC daily snow (inches) plot generation complete "
        ...
        gif_file_out = (
            f"wrf_{domain}_Daily_Total_Snow_Inch_SLP_Isotherm_{cycle_label}.gif"
        )
