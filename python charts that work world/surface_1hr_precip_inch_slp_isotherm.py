...
def process_frame(args):
    ...
    Steps:
        ...
        * Compute 1-hour accumulation as difference of cumulative totals.
        * Build map (features, cities, gridlines).
        * Plot:
            - Smoothed SLP contours (hPa) with H/L markers.
            - 1-hour accumulated precip (inches) as filled contours.
    ...
    with Dataset(ncfile_path) as ncfile:
        ...
        # 2-m temperature (smoothed) is retained but not plotted, to keep physics
        ...
        cbar.set_label("1-hour Total Precipitation (in)", fontsize=14)
        ...
        file_out = f"wrf_{domain}_SLP_1hrPrecip_{fname_time}.png"
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 3:
        print(
            ...
            "For example:\n"
            "    SFC_1hrPrecip_SLP.py /home/WRF/test/em_real d01\n"
        )
        sys.exit(1)
    ...
    # -------------------------------------------------------------------------
    path_figures = "wrf_SFC_1hrPrecip_Inch_figures"
    image_folder = os.path.join(path_figures, "Images")
    ...
    # -------------------------------------------------------------------------
    # Build list of frames and previous-frame links
    # -------------------------------------------------------------------------
    ...
    # Build args_list including previous frame info
    args_list = []
    ...
    print("SFC SLP + 1-hour precipitation plot generation complete.")
    ...
    gif_file_out = f"wrf_{domain}_1-hour_Total_Precip_SLP.gif"
