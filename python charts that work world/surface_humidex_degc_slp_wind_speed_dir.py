...
and surface Humidex (°C) on a Cartopy map.
...
###############################################################################
# Humidex physics (immutable)
###############################################################################
def calculate_humidex(T_air_C, T_dew_C):
    """
    Calculate the Canadian Humidex given air temperature and dew point in °C.

    Physics/formula kept identical to original script.
    """
    # Vapour pressure in hPa
    e = 6.11 * np.exp(5417.7530 * (1 / 273.15 - 1 / (273.15 + T_dew_C)))
    # Humidex formula
    Humidex = T_air_C + 0.5555 * (e - 10)
    return Humidex
...
def process_frame(args):
    ...
    Steps:
        * Read WRF variables (slp, U10, V10, T2, td2) at this time.
        * Compute Humidex from T2 and td2 (°C).
        * Smooth SLP.
        * Build map (features, cities, gridlines).
        * Plot:
            - SLP contours (hPa).
            - Humidex (°C) filled contours.
    ...
    with Dataset(ncfile_path) as ncfile:
        ...
        td2 = wrf.getvar(ncfile, "td2", timeidx=time_index, units="degC")

        u10_knots = u10 * 1.94384449
        v10_knots = v10 * 1.94384449

        # Convert temp to °C (physics preserved)
        temp_c_raw = temp - 273.15

        # Smooth temperature and dewpoint for Humidex calculation
        temp_c_smooth = gaussian_filter(to_np(temp_c_raw), sigma=1.0)
        td2_smooth = gaussian_filter(to_np(td2), sigma=1.0)

        # Calculate Humidex (°C)
        humidex = calculate_humidex(temp_c_smooth, td2_smooth)

        # Convert wind speed from m/s to knots (unchanged physics)
        wind_speed_knots = np.sqrt((to_np(u10_knots)) ** 2 + (to_np(v10_knots)) ** 2)

        # -------------------------------------------------------------------------
        # Lat/lon, grid spacing, extent padding – per frame (moving-nest safe)
        # -------------------------------------------------------------------------
        ...
        # -------------------------------------------------------------------------
        # Smooth SLP and plot highs/lows
        # -------------------------------------------------------------------------
        ...
        # -------------------------------------------------------------------------
        # Humidex and SLP contouring (physics invariant)
        # -------------------------------------------------------------------------
        H_Levels = np.array([20, 30, 40, 45, 60])
        ...
        H_map = plt.matplotlib.colors.ListedColormap(color_map_rgb)
        H_norm = plt.matplotlib.colors.BoundaryNorm(H_Levels, H_map.N)
        ...
        # Humidex filled contours
        humidex_plot = ax.contourf(
            lons_np,
            lats_np,
            humidex,
            levels=H_Levels,
        ...
        # -------------------------------------------------------------------------
        # Wind barbs with brightness-aware coloring (physics preserved)
        # -------------------------------------------------------------------------
        ...
        # Brightness of colormap
        color_brightness = np.dot(color_map_rgb, [0.299, 0.587, 0.114])
        brightness_threshold = 0.4

        norm = plt.Normalize(vmin=H_Levels[0], vmax=H_Levels[-1])
        H_normalized = norm(humidex)

        brightness_map = np.interp(
            H_normalized, np.linspace(0, 1, len(color_brightness)), color_brightness
        )

        dark_region = brightness_map <= brightness_threshold
        light_region = brightness_map > brightness_threshold

        # NOTE: Outside/inside mask logic kept identical,
        # even though it uses temp instead of humidex.
        outside_contour_mask = (
            (temp_np < H_Levels[0]) | (temp_np > H_Levels[-1]) | np.isnan(temp_np)
        )
        ...
        # -------------------------------------------------------------------------
        cbar2 = fig.colorbar(
            humidex_plot, ax=ax, orientation="vertical", shrink=0.8, pad=0.05
        )

        # Custom ticks at boundaries + midpoints (unchanged)
        cbar2.set_ticks([20, 30, 40, 45, 60])
        ...
        cbar2.set_label("Humidex (°C) and Comfort Levels", fontsize=14)
        ...
        # -------------------------------------------------------------------------
        # Save figure with valid time timestamp for GIF sorting
        # -------------------------------------------------------------------------
        fname_time = valid_dt.strftime("%Y%m%d%H%M%S")
        file_out = f"wrf_{domain}_slp_wind_humidex_{fname_time}.png"
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 3:
        print(
            ...
            "For example:\n"
            "    SFC_Humidex.py /home/WRF/test/em_real d01\n"
        )
        sys.exit(1)
    ...
    # -------------------------------------------------------------------------
    path_figures = "wrf_SFC_Humidex_degC_figures"
    ...
    # -------------------------------------------------------------------------
    # Discover all frames and build argument list
    # -------------------------------------------------------------------------
    ...
    print("SFC Humidex plots complete.")
    ...
    # Alphabetical sort works because filenames contain YYYYMMDDHHMMSS
    ...
    gif_file_out = f"wrf_{domain}_SLP_WIND_Humidex.gif"
